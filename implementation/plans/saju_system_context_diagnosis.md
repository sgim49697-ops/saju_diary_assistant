...
- 이번 요청은 새 계획과 문서·테스트의 정합화다. 진단 코드 구현, 프롬프트 교체, 모델 다운로드·GPU 실행, teacher 호출, 데이터 생성·400건 재개·학습, 서비스 전환·브랜치 병합은 실행하지 않는다.
...
| [00 기준선](saju_product_roadmap/00-current-baseline.md) | 원격 master·v1.14 운영·v1.15 후보·문서 브랜치가 다름; 400건 보정은 현재 R16에 미반영 | 계획이나 후보 코드가 이미 운영에 적용됐다는 가정 |
