...
- `saju_product_roadmap/00-current-baseline.md`: 원격 master·v1.14 운영·v1.15~v1.19 부모·v1.20 CPU 후보와 S5 분석 완료를 구분하는 현재 상태 정본
...
| 최신 대화 진단 | S0/S1·S2·CPU 재집계 보존, S3 96·S4 192요청·S5 CPU 분석 검증 완료 | S4 후보 미채택·첫 실패 보존; S5 `build-27a91a8e21a8` 가설/조건부 명세만 완료·S6 미실행 |
| 앱 | dashboard v1.14 운영 / v1.18·v1.19 부모 보존·v1.20 코드 검토 보완 CPU/합성 화면 검증 완료 | `build-bc8f19f91297`, 실제 모델 생성 0·현재 service·기본 모델·feature off 유지 |
| 후속 계획 | Phase 7 정본화·8A 후보·8B S3·9 S4·10 CPU·11 분석 완료, 12 미실행·13~14 조건부 보류 | 다음 Phase 12, 기존 Phase·불변 산출물·release·후속 실행 권한 불변 |
...
- 2026-09-17: [Claude Code 코드 검토·v1.20 보완](../history/2026-09-17-claude-product-review.md#claude-review)을 현재 후보로 연결했다. S5와 부모 결과는 보존하고 다음 Phase 12·보정/학습/운영 보류를 유지한다.

### 2026-09-16 — S4 공식 pin·실행기·CPU 검증 연결

- [Phase 9 구현 기록](../history/2026-09-16-phase9-s4.md)에 현재 구현·테스트·실행 제한을 연결했다. 실제 모델 비교는 미실행이며 원문 반영표·기존 불변 산출물·운영 앱을 보존했다.
