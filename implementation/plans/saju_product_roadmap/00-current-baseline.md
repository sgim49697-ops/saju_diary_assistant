...
기준 commit `390ca88f4b65f6c3b87f0f36ca26cbc492251a06`에서 [원문 3개·841줄·80개 섹션](source-20260915.md)을 [Phase 7~14](README.md)에 통합했다. Phase 7은 문서·테스트 완료, 8A는 후보 CPU 확인 완료, 8B는 S3 실행·검증 완료, 9는 실행기·CPU 검증 완료/실제 비교 미실행, 10~12는 미실행, 13~14는 조건부 보류다. 다음은 Phase 9의 실제 수집·비교 범위 확인이다. [8A 실행 기록](../../history/2026-09-16-phase8-intent-s3.md#phase8a)은 이전 25개에 더해 새 정책·경로·보고서와 화면을 검증했다. 기존 Phase 0~6·S0~S2 완료와 승인 상태는 바뀌지 않는다.
...
| S4 다음 비교 준비 | 공식 3B pin·192요청 실행기·scorer v1.2 | CPU 검증 완료; 가중치 다운로드·실제 비교 미실행 |
...
| 최신 전체 경로 진단 | S2·CPU 재집계·S3 실행·검증 완료 | S4가 다음 별도 과제, S4~S6 미실행 |
...
- [S4 구현 기록](../../history/2026-09-16-phase9-s4.md)은 K0/3B×FULL/MIN·공통 P0 비교 준비다. 실제 3B는 native Qwen3 구조이며 기존 K0 loader를 재명명하지 않았다. 공식 pin·CPU 회귀만 확인했고 실제 모델 성능·최종 tokenizer 렌더링·warm 운영 비용은 미측정이다. 새 GPU 생성 0, 잔여 예산 242를 유지한다.
