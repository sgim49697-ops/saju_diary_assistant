...
이 문서는 후속 로드맵의 **현재 상태 정본**이다. 2026-09-17 Claude Code 코드 검토에 따른 Phase 10 v1.20 R16 보완과 Phase 11 S5 전수 대조·조건부 학습 명세 완료를 반영한다. Phase 8A·S3·S4, S0~S2·CPU 재집계·v1.16 및 PR #28·기존 학습·응답·데이터 이력은 보존한다. 실행 순서는 [로드맵 README](README.md), 진단 상세는 [50번 문서](50-automatic-model-evaluation.md)가 소유한다.

## Phase 7 정본화와 다음 실행

기준 commit `390ca88f4b65f6c3b87f0f36ca26cbc492251a06`에서 [원문 3개·841줄·80개 섹션](source-20260915.md)을 [Phase 7~14](README.md)에 통합했다. Phase 7은 문서·테스트 완료, 8A는 후보 CPU 확인 완료, 8B는 S3 실행·검증 완료, 9는 S4 v1.1 192요청·검증 완료·후보 미채택, 10은 R16 제품 후보 CPU 검증 완료, 11은 S5 분석·가설·조건부 명세 완료, 12는 미실행, 13~14는 조건부 보류다. 다음은 Phase 12의 전체 후보 구성 동결·새 질문 실제 확인이다. [첫 S4 중단](../../history/2026-09-16-phase9-s4-execution.md#blocked-run)의 실패 1건은 [별도 승인 v1.1 결과](../../history/2026-09-16-phase9-s4-recovery.md#phase9)와 분리해 보존한다. [8A 실행 기록](../../history/2026-09-16-phase8-intent-s3.md#phase8a)은 이전 25개에 더해 새 정책·경로·보고서와 화면을 검증했다. 기존 Phase 0~6·S0~S2 완료와 승인 상태는 바뀌지 않는다.
...
| 최신 앱 후보 | v1.20 R16, CPU canary `build-bc8f19f91297` | 71개 CPU·32개 합성 화면 통과, 실제 모델 생성 0·운영 미배포 |
| 최신 데이터 대조 | S5 `build-27a91a8e21a8` | 2K 전수 token/mask 재현·7축 가설·400건 보류, 새 생성/학습 0 |
...
| 최신 전체 경로 진단 | S2·CPU 재집계·S3·S4·S5 실행·검증 완료 | Phase 10 후보 CPU 검증·11 분석 완료, 다음 12/S6 미실행 |
...
| 앱 후보 | v1.12~v1.19 부모 보존, v1.20 제품 CPU canary 완료 | 기본 포트 8772·별도 세션·기능 기본 off, 현재 서비스는 v1.14 |
...
- 별도 v1.1 보정 `repair-23340fc31022`는 S5 읽기 전용 재검증 기준 accepted 238/400, 판정 대기 3, 초안·재작성 대기 159다. 현재 R16에 반영되지 않았다. 실제 계약은 1,600행 계승+400행 교체=2,000행이다. 연결 일반 대화/전환을 직접 보완하는 축이 아니므로 Phase 12 실제 확인 전까지 보류하며 다른 세션 state는 수정하지 않는다.
- [S5 전수 대조](../../history/2026-09-17-phase11-data-hypotheses.md#phase11)는 기존 token/mask/EOS 2,000행 재현 일치·잘림/누출 0, 공감 250행 미연결 단일 턴·3줄 이상 답변 1,751행을 확인했다. 기존 dev 200의 마지막 질문은 학습과 모두 겹치지만 부모 대화/원국 fingerprint 일치는 0이다. 새 질문 일반화와 의미 품질은 미측정이며 7축 가설·조건부 명세만 완료했다.
...
최신 후보 검증은 [v1.20 Claude Code 코드 검토·보완 기록](../../history/2026-09-17-claude-product-review.md#claude-review)을 따른다. [v1.19 버그 보완 기록](../../history/2026-09-17-phase10-product-fixes.md#phase10-fixes)과 S5의 당시 후보 식별자는 보존했다. [v1.17](../../history/2026-09-16-phase8-intent-s3.md#phase8a)도 같은 source hash로 재검증했다. [v1.16](../../history/2026-09-15-dashboard-v116-intent.md)·[v1.15](../../history/2026-09-05-dashboard-v115-grounding.md)·[20문장](../../history/2026-09-05-dashboard-prompt20.md)은 부모 이력으로 보존한다. private artifact가 없는 격리 환경에서 생기는 전체 회귀 오류를 성공으로 기록하지 않는다.
...
- 2026-09-17 Claude Code 코드 검토: 최신 CPU 후보를 v1.20으로 연결하고 v1.19·S5를 같은 해시로 재검증했다. 실제 서비스는 PID 3144071·재시작 0·v1.14를 유지하며 변경 범위·한계는 [코드 검토 기록](../../history/2026-09-17-claude-product-review.md#claude-review)을 따른다.

- 2026-09-17: v1.19 버그 보완·S5 `build-27a91a8e21a8`을 현재 기준선에 연결했다. [Phase 11 기록](../../history/2026-09-17-phase11-data-hypotheses.md#phase11)의 데이터/teacher 전수 대조와 조건부 명세를 반영하고 다음을 Phase 12로 지정했다. 추가 모델 요청·학습·운영 변경은 없다.
