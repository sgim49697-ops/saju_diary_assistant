...
기준 commit `390ca88f4b65f6c3b87f0f36ca26cbc492251a06`에서 [원문 3개·841줄·80개 섹션](source-20260915.md)을 [Phase 7~14](README.md)에 통합했다. Phase 7은 문서·테스트 완료, 8A는 후보 CPU 확인 완료, 8B는 S3 실행·검증 완료, 9는 모델 수집·dry-run 뒤 첫 요청 오류로 blocked, 10~12는 미실행, 13~14는 조건부 보류다. 다음은 Phase 9의 소비 계약 호환성 수정·새 실행 범위 결정이다. [S4 중단 기록](../../history/2026-09-16-phase9-s4-execution.md#blocked-run)의 1오류·191미실행을 실제 비교 완료로 세지 않는다. [8A 실행 기록](../../history/2026-09-16-phase8-intent-s3.md#phase8a)은 이전 25개에 더해 새 정책·경로·보고서와 화면을 검증했다. 기존 Phase 0~6·S0~S2 완료와 승인 상태는 바뀌지 않는다.
...
| 최신 전체 경로 진단 | S2·CPU 재집계·S3 실행·검증 완료 | S4 첫 요청 오류로 중단, S5~S6 미실행 |
...
- [S4 실제 실행](../../history/2026-09-16-phase9-s4-execution.md#blocked-run)은 공식 3B 수집·두 tokenizer 입력 검증까지 통과했으나 첫 K0 요청의 소비 계약 충돌로 중단됐다. 오류 1·정상 응답 0·191미실행이며 3B GPU 요청은 0이다. 비교 성능·비용은 미측정이고 잔여 산술은 241이다. [구현 기록](../../history/2026-09-16-phase9-s4.md)의 CPU 성공을 실제 경로 성공으로 확대하지 않는다.
