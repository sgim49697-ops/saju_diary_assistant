...
| 기준일 | 2026-09-15; S2 `build-c39b4bce5089` 완료·과거 응답 이력 유지 |
| 현재 Runtime | `saju-runtime-release-v1.5.0-8b1d6ea2d46e` |
| 앱 상태 | dashboard v1.14 운영 / v1.15 검증 후보·병합 완료·운영 미배포 |
| 다음 실행 축 | S1 검사 보강·S2 파생 재집계 제안 → 50-B2 지시문 → 50-C 크기×컨텍스트 → 50-D 데이터·학습·새 질문 확인 |
...
| 50 | [`50-automatic-model-evaluation.md`](50-automatic-model-evaluation.md) | A/S0·S1 계약 검증·B1/S2 실행 완료 / 검사 보강 제안 | B2/S3~D/S6 미실행, 원인별 한계·다음 조치 기록 |
...
10~40은 완료 이력으로 보존하며 다시 구현하지 않는다. 50-A의 S0/S1은 계약·CPU 검증을, 50-B1/S2는 342요청 실행·재검증을 마쳤다. 다음 최소 제안은 검사기 오탐의 새 버전 보강과 기존 합성 응답의 파생 재집계이며 추가 GPU 생성이 필요하지 않다. 그 뒤 S3 지시문·S4 크기 비교를 이어간다. 입력이 잘리지 않았다는 결과만으로 컨텍스트 간섭을 배제하지 않는다. 50-C의 큰 동일 계열 Instruct 기본 모델 비교는 필수 진단 축이지만 이번 실행 범위 밖이다. 정확한 후보 등록과 별도 실행 결정 전 큰 모델 다운로드·GPU 실행은 하지 않는다.
