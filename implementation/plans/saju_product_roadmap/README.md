...
| 항목 | 값 |
|---|---|
| 문서 버전 | `saju-product-roadmap-v1.4.0` |
| 진단 기준 | `26462137f9a4ef34adb2d3db0dd6eaff6282b309`의 완료 산출물 |
| 기준일 | 2026-09-16; 8A·S3·S4 보존 및 Phase 10 `build-f13715ee1d91` CPU/합성 화면 검증 |
| 현재 Runtime | `saju-runtime-release-v1.5.0-8b1d6ea2d46e` |
| 앱 상태 | dashboard v1.14 운영 / v1.15~v1.17 보존 / v1.18 R16 제품 후보 CPU 검증·운영 미배포 |
...
| 10 | [응답 모드·정정 흐름](phases/phase-10.md) | 최소 후보 구현·CPU 검증 완료, 운영 미배포 | R16/v1.18, `build-f13715ee1d91`: CPU 46·합성 화면 16·실제 모델 생성 0 |
...
10~70은 기존 문서 번호이며 Phase 번호가 아니다. 10~40은 완료 이력으로 보존하며 다시 구현하지 않는다. 50-A의 S0/S1·50-B1/S2·CPU 재집계와 **S3: 고정 R16/C_FULL·부모 이력에서 P0/P1 지시문만 비교**하는 Phase 8B, 공통 P0의 K0/3B×FULL/MIN인 Phase 9를 마쳤다. P1은 파일과 formatter 추가 지시를 포함한 묶음 하나이며 사실 JSON을 고정했다. 입력 무삭제는 간섭 부재의 증거가 아니다. S4의 첫 실패와 v1.1 완료를 분리하고 모델 품질·운영 승격을 자동 승인하지 않는다. 최신 v1.18은 R16 단독 최소 제품 후보로 CPU·합성 화면 검증을 완료했다. v1.17 부모 보존·운영 미배포이며 실제 모델 품질은 아직 측정하지 않았다.
