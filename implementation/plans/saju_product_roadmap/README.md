...
| 항목 | 값 |
|---|---|
| 문서 버전 | `saju-product-roadmap-v1.3.0` |
...
완료된 Phase 0~6은 초기 학습 이력이며 번호·상태를 변경하지 않는다. 아래 문서는 동일 로드맵 안의 작업 단위다. [검토 원문과 전량 반영표](source-20260915.md)의 3개 문서·841줄·80개 섹션·588개 비어 있지 않은 행을 담당 절에 연결했다. 원문은 archive로 보존하고 병렬 정본으로 추가하지 않는다.

| Phase | 문서 | 현재 상태 | 완료 산출물 |
|---:|---|---|---|
...
| 11 | [S5 데이터·학습 결정](phases/phase-11.md) | 미실행 | 400건 유지/재설계/보류·학습 필요성·최소 범위 |
...
8A와 8B는 독립 작업이다. **8B → 9**이며 앱 수정이나 Phase 10 전체 구현을 기다리느라 S4를 미루지 않는다. 8A+9의 결과로 10 → 11 → 12를 진행한다. 13은 조건부 분기이며 새 학습을 했으면 새 검증 후 14로 간다. S3 반복·R8/R32 추가 비교·20K 학습을 S4의 선행 조건으로 추가하지 않는다. 계산기는 [동결·유지보수·재개 조건](phases/phase-07.md#reopen)만 열어 두며 기본 개발 중심은 앱·모델이다.

S3/S4/S6 본 비교는 336요청이다. 기존 680 상한에서 부모 342요청(적격성 6 포함)을 대조하면 공유 적격성 잔여는 2다. [예산·실행 권한](phases/phase-07.md#budget)은 별개이며 문서만으로 후속 실행을 승인하지 않는다. 새 API·CLI는 구현 전까지 예정이다.
