...
| 기준일 | 2026-09-14; 완료 응답 기준선은 2026-09-05 기록 유지 |
| 현재 Runtime | `saju-runtime-release-v1.5.0-8b1d6ea2d46e` |
| 앱 상태 | dashboard v1.14 운영 / v1.15 검증 후보·병합 완료·운영 미배포 |
| 다음 실행 축 | 50-A 전체 경로·계약 → 50-B 컨텍스트/지시문 분리 → 50-C 크기×컨텍스트 → 50-D 데이터·학습·새 질문 확인 |
...
10~40은 완료 이력으로 보존하며 다시 구현하지 않는다. 다음 작업은 50-A에 대응하는 새 계획의 S0/S1, 즉 실행 기준선·전체 경로 trace·CPU 자동 계약이다. 입력이 잘리지 않았다는 결과만으로 컨텍스트 간섭을 배제하지 않는다. 50-C의 큰 동일 계열 Instruct 기본 모델 비교는 필수 진단 축이며 선택 항목으로 건너뛰지 않는다. 정확한 모델 등록과 실행 가능성 확인 전 다운로드·GPU 실행은 하지 않는다.
