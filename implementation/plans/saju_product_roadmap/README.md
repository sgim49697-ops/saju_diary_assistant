...
| 항목 | 값 |
|---|---|
| 문서 버전 | `saju-product-roadmap-v1.2.1` |
...
| 다음 실행 축 | 50-B2/S3 지시문 → 50-C/S4 크기×컨텍스트 → 50-D 데이터·학습·새 질문 확인; 별도 승인 |
...
6. [전체 흐름·컨텍스트 진단 계획](../saju_system_context_diagnosis.md)은 50이 위임한 실험 설계·변수·실행 파일 순서를 소유한다. 50-A~D 안의 S0~S6이며 별도 Phase나 승인 Gate가 아니다.
...
10~40은 완료 이력으로 보존하며 다시 구현하지 않는다. 50-A의 S0/S1·50-B1/S2와 검사기 후속·CPU 파생 재집계를 마쳤다. 다음 최소 작업은 **S3: 고정 R16/C_FULL·부모 이력에서 P0/P1 지시문만 비교**하는 것이다. 후보 하나와 실행 계약을 먼저 고정하며 자동 실행하지 않는다. 입력이 잘리지 않았다는 결과만으로 컨텍스트 간섭을 배제하지 않는다. 이어질 S4의 큰 동일 계열 Instruct 기본 모델 비교는 별도 등록·승인 전 다운로드하거나 실행하지 않는다. 앱 v1.16은 CPU 검증 후보이며 이번 진단으로 운영에 배포하지 않는다.
