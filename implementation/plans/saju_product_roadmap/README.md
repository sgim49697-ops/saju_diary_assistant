...
| 다음 실행 축 | Phase 8A 후보 CPU·8B S3 완료 → 다음 Phase 9 S4 → Phase 10~14; 후속 실행 별도 |
...
| 9 | [S4 규모×정보](phases/phase-09.md) | 미실행 | K0 / Kanana 3B Instruct, 공통 P0·FULL/MIN 최대 192요청 |
...
10~70은 기존 문서 번호이며 Phase 번호가 아니다. 10~40은 완료 이력으로 보존하며 다시 구현하지 않는다. 50-A의 S0/S1·50-B1/S2·CPU 재집계와 **S3: 고정 R16/C_FULL·부모 이력에서 P0/P1 지시문만 비교**하는 Phase 8B를 마쳤다. P1은 파일과 formatter 추가 지시를 포함한 묶음 하나이며 사실 JSON을 고정했다. 입력 무삭제는 간섭 부재의 증거가 아니다. 다음 S4의 첫 후보는 Kanana 3B Instruct지만 별도 등록·승인 전 다운로드하지 않는다. 최신 v1.17은 CPU·합성 화면 검증 후보이며 운영 미배포다.
