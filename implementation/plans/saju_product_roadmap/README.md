...
| 기준일 | 2026-09-15; S2와 CPU 재집계 `build-8547c487c858` 검증·과거 응답 보존 |
...
| 다음 실행 축 | Phase 8A 후보 CPU 완료 / 8B S3 실행 준비 → Phase 9 S4 → Phase 10~14; 후속 실행 별도 |
...
| 8 | [앱 오차단 / S3 지시문](phases/phase-08.md) | 8A 후보 CPU 완료·8B 준비 | 45개 CPU·6개 합성 화면 확인, 8B 최대 96요청 비교와 분리 |
...
계획상 잔여는 **338요청 = 본 비교 336 + 적격성 잔여 2**다. 산식·추가 모델 호출·Phase 13 학습 후 별도 평가 예산은 [예산·실행 권한](phases/phase-07.md#budget)이 소유한다. 이는 실행 승인이 아니며 CPU 모의 검증과 실제 모델 호출을 구분한다. 새 API·CLI는 구현 전까지 예정이다.

## 현재 판정

- 최신 후보·진단·계획은 PR #28 `3bb0ce2`로 `master`에 병합했고 원본 폴더도 동기화했다. 후속 작업은 이 기본 브랜치에서 진행하며 새 분기는 사전 확인 없이 만들지 않는다.
...
| 50 | [`50-automatic-model-evaluation.md`](50-automatic-model-evaluation.md) | A/S0·S1·B1/S2와 검사 보강·CPU 파생 재집계 완료 | B2/S3~D/S6 미실행, 원인별 한계·다음 조치 기록 |
...
10~70은 기존 문서 번호이며 Phase 번호가 아니다. 10~40은 완료 이력으로 보존하며 다시 구현하지 않는다. 50-A의 S0/S1·50-B1/S2와 검사 보강·CPU 재집계를 마쳤다. 다음은 Phase 8A 앱 오차단과 **S3: 고정 R16/C_FULL·부모 이력에서 P0/P1 지시문만 비교**하는 Phase 8B다. P1은 파일과 formatter 추가 지시를 포함한 묶음 하나이며 사실 JSON은 고정한다. 입력 무삭제는 간섭 부재의 증거가 아니다. S4의 첫 후보는 Kanana 3B Instruct지만 별도 등록·승인 전 다운로드하지 않는다. v1.16은 CPU 검증 후보이며 운영 미배포다.
