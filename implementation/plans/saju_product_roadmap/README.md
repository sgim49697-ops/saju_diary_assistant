...
| 다음 실행 축 | Phase 9 S4 실행기·CPU 검증 완료, 실제 비교 미실행 → 결과 확인 후 Phase 10~14 |
...
| 9 | [S4 규모×정보](phases/phase-09.md) | 실행기·CPU 검증 완료, 실제 비교 미실행 | K0 / Kanana 3B Instruct, 공통 P0·FULL/MIN 최대 192요청 |
...
S3 96요청 완료 후 계획상 잔여는 **242요청 = 본 비교 240 + 적격성 잔여 2**다. Phase 7 당시 338요청 산식·추가 모델 호출·Phase 13 학습 후 별도 평가 예산은 [예산·실행 권한](phases/phase-07.md#budget)이 소유한다. 이는 실행 승인이 아니며 CPU 모의 검증과 실제 모델 호출을 구분한다. 새 API·CLI는 구현 전까지 예정이다.
...
- 확정 20문장 비교는 60요청·54생성·6차단으로 완료됐다. 구조 검사 통과 수는 모델 정확도가 아니며, 이후 지시문 비교는 S3에서 별도로 완료했다. 큰 기본 모델 비교는 아직 실행하지 않았다.
...
| 50 | [`50-automatic-model-evaluation.md`](50-automatic-model-evaluation.md) | A/S0·S1·B1/S2·CPU 파생·B2/S3 완료 | C/S4~D/S6 미실행, P1 미채택·한계·다음 조치 기록 |
...
10~70은 기존 문서 번호이며 Phase 번호가 아니다. 10~40은 완료 이력으로 보존하며 다시 구현하지 않는다. 50-A의 S0/S1·50-B1/S2·CPU 재집계와 **S3: 고정 R16/C_FULL·부모 이력에서 P0/P1 지시문만 비교**하는 Phase 8B를 마쳤다. P1은 파일과 formatter 추가 지시를 포함한 묶음 하나이며 사실 JSON을 고정했다. 입력 무삭제는 간섭 부재의 증거가 아니다. S4의 Kanana 3B 공식 pin과 실행기를 등록했지만 수집·GPU 범위 확인 전 다운로드하지 않는다. 최신 v1.17은 CPU·합성 화면 검증 후보이며 운영 미배포다.
