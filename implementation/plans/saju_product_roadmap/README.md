...
| 항목 | 값 |
|---|---|
| 문서 버전 | `saju-product-roadmap-v1.3.1` |
| 진단 기준 | `26462137f9a4ef34adb2d3db0dd6eaff6282b309`의 완료 산출물 |
| 기준일 | 2026-09-16; 8A CPU·합성 화면 및 S3 `build-ffd985905b51` 검증·부모 응답 보존 |
| 현재 Runtime | `saju-runtime-release-v1.5.0-8b1d6ea2d46e` |
| 앱 상태 | dashboard v1.14 운영 / v1.15·v1.16 보존 / v1.17 오차단 후보 CPU 검증·운영 미배포 |
| 현재 문서 작업 | Phase 7 정본화 완료·계산기 범위 동결; 기준 `390ca88`, 원문 3개·841줄 전량 반영 |
| 다음 실행 축 | Phase 9 S4 첫 요청 저장 계약 오류로 중단 → 호환성 수정·새 실행 범위 결정; Phase 10~14 미착수 |
...
| 9 | [S4 규모×정보](phases/phase-09.md) | 모델 수집·dry-run 완료, 첫 요청 오류로 blocked | `build-296dffd1ef51`: 1오류·191미실행, 유효 비교 없음 |
| 10 | [응답 모드·정정 흐름](phases/phase-10.md) | 미실행 | 연결 상태와 응답 분리·직접 조회·정보/이력 선택 후보 하나 |
...
S3 완료 후 242요청 중 S4 실패 1요청을 소비해 계획상 잔여는 **241요청 = S4 미처리 191 + S6 48 + 여유 2**다. 실패 build의 미처리분은 자동 재개 권한이 아니며 192요청 재실험은 추가 1요청의 목적·범위를 별도로 확인해야 한다. Phase 7 당시 338요청 산식·추가 모델 호출·Phase 13 학습 후 별도 평가 예산은 [예산·실행 권한](phases/phase-07.md#budget)이 소유한다. CPU 모의 검증과 실제 모델 호출을 구분한다. 새 API·CLI는 구현 전까지 예정이다.
...
- 확정 20문장 비교는 60요청·54생성·6차단으로 완료됐다. 구조 검사 통과 수는 모델 정확도가 아니며, 이후 지시문 비교는 S3에서 별도로 완료했다. 큰 기본 모델 비교는 첫 K0 요청 오류로 중단돼 유효한 결과가 없다.
...
- [Phase 9 구현 기록](../../history/2026-09-16-phase9-s4.md)은 당시의 공식 3B 등록·CPU 검증 이력이다. 이후 [실제 실행](../../history/2026-09-16-phase9-s4-execution.md#blocked-run)에서 수집·dry-run은 통과했지만 첫 요청의 소비 계약 오류로 중단됐다. 3B GPU 요청은 0이며 학습·운영 권한은 그대로다.
...
| 50 | [`50-automatic-model-evaluation.md`](50-automatic-model-evaluation.md) | A/S0·S1·B1/S2·CPU 파생·B2/S3 완료 | C/S4 중단, D/S5~S6 미실행; P1 미채택·한계·다음 조치 기록 |
...
10~70은 기존 문서 번호이며 Phase 번호가 아니다. 10~40은 완료 이력으로 보존하며 다시 구현하지 않는다. 50-A의 S0/S1·50-B1/S2·CPU 재집계와 **S3: 고정 R16/C_FULL·부모 이력에서 P0/P1 지시문만 비교**하는 Phase 8B를 마쳤다. P1은 파일과 formatter 추가 지시를 포함한 묶음 하나이며 사실 JSON을 고정했다. 입력 무삭제는 간섭 부재의 증거가 아니다. S4의 공식 모델 수집·입력 검증과 첫 요청의 저장 오류를 실제 비교 완료와 구분한다. 최신 v1.17은 CPU·합성 화면 검증 후보이며 운영 미배포다.
