...
| 항목 | 값 |
|---|---|
| 문서 버전 | `saju-product-roadmap-v1.7.0` |
| 진단 기준 | `26462137f9a4ef34adb2d3db0dd6eaff6282b309`의 완료 산출물 |
| 기준일 | 2026-09-17; Claude Code 코드 검토·Phase 10 v1.20 보완·Phase 11 S5 전수 대조 보존 |
| 현재 Runtime | `saju-runtime-release-v1.5.0-8b1d6ea2d46e` |
| 앱 상태 | dashboard v1.14 운영 / v1.15~v1.19 보존 / v1.20 R16 제품 후보 CPU 검증·운영 미배포 |
...
4. 이 README는 위 사실을 바꾸지 않고 **전체 실행 순서**를 소유한다. 완료 Phase 0~6의 계약은 초기 인덱스를 참조하고 이후 순서는 아래 Phase 7~14를 따른다. 같은 디렉터리의 00 문서는 **현재 상태**, 50 문서는 **원인 분리 진단 상세**를 소유한다.
...
6. [전체 흐름·컨텍스트 진단 계획](../saju_system_context_diagnosis.md)은 50이 위임한 실험 설계·변수·실행 파일 순서를 소유한다. S0~S6·50-A~D는 실험 번호로 유지한다. 사용자 선택으로 후속 실행을 Phase 7~14에 배치하되 별도 승인 Gate는 만들지 않는다.

## Phase 7~14 실행 순서

완료된 Phase 0~6은 초기 학습 이력이며 번호·상태를 변경하지 않는다. 아래 문서는 동일 로드맵 안의 작업 단위다. [검토 원문과 전량 반영표](source-20260915.md)의 3개 문서·841줄·80개 섹션·588개 비어 있지 않은 행을 담당 절에 연결했다. 요구사항 유형·문서 반영 상태·실행 상태는 별개이며 Phase 7 완료는 문서 조건·예외까지 반영·검증했다는 뜻이지 후속 구현 완료가 아니다. 원문은 archive로 보존하고 병렬 정본으로 추가하지 않는다.

| Phase | 문서 | 실제 진행 상태 | 완료 시 필요한 산출물 |
|---:|---|---|---|
| 7 | [정본화·계산기 동결](phases/phase-07.md) | 완료 | 전량 반영·문서 정합성·검증·한국어 저장 |
...
| 10 | [응답 모드·정정 흐름](phases/phase-10.md) | 최소 후보 구현·CPU 검증 완료, 운영 미배포 | R16/v1.20, `build-bc8f19f91297`: CPU 71·합성 화면 32·실제 모델 생성 0 |
...
8A와 8B는 독립 작업이다. **8B → 9**이며 앱 수정이나 Phase 10 전체 구현을 기다리느라 S4를 미루지 않는다. 8A+9의 결과로 10 → 11(가설·조건부 명세) → 12(실제 후보 확인·학습 실행 여부 판단)를 진행한다. 13은 12의 결과를 대조한 뒤 별도 결정하는 조건부 분기다. 앱·지시문으로 해소됐으면 학습을 건너뛰고, 새 학습을 했으면 별도 새 검증 후 14로 간다. S3 반복·R8/R32 추가 비교·20K 학습을 S4의 선행 조건으로 추가하지 않는다. 계산기는 [동결·유지보수·재개 조건](phases/phase-07.md#reopen)만 열어 두며 기본 개발 중심은 앱·모델이다.

S3까지 438요청 + S4 실패 1요청 + 별도 승인된 v1.1 비교 192요청으로 누적 **631/680**, 현재 잔여는 **49요청 = S6 48 + 공유 여유 1**이다. 실패 증거·기존 원장은 보존했고 CPU 실제 저장 재생은 모델 호출로 세지 않는다. Phase 7 당시 338요청 산식·추가 호출·Phase 13 학습 후 별도 평가 예산은 [예산·실행 권한](phases/phase-07.md#budget)이 소유한다. 잔여 예산은 후속 실행 승인이 아니다. 새 API·CLI는 구현 전까지 예정이다.

## 현재 판정

- v1.15 후보·진단·계획은 PR #28 `3bb0ce2`로 `master`에 병합했고 원본 폴더도 동기화했다. 이후 8A `c1f090f`·S3 실행기 `2d951b4`도 원본 `master`에서 검증·푸시했다. 새 분기는 사전 확인 없이 만들지 않는다.
...
- 확정 20문장 비교는 60요청·54생성·6차단으로 완료됐다. 구조 검사 통과 수는 모델 정확도가 아니며, 이후 지시문 S3와 큰 기본 모델 S4 비교를 별도로 완료했다.
- 새 합성 48개 기반 S2는 342요청·312생성·30사전 차단과 재구성 검증을 완료했다. 검사기 오탐을 확인했으므로 FULL/MIN·모델 우열은 확정하지 않는다. [완료 기록](../../history/2026-09-15-system-context-diagnosis.md)을 따른다.
...
10~70은 기존 문서 번호이며 Phase 번호가 아니다. 10~40은 완료 이력으로 보존하며 다시 구현하지 않는다. 50-A의 S0/S1·50-B1/S2·CPU 재집계와 **S3: 고정 R16/C_FULL·부모 이력에서 P0/P1 지시문만 비교**하는 Phase 8B, 공통 P0의 K0/3B×FULL/MIN인 Phase 9를 마쳤다. P1은 파일과 formatter 추가 지시를 포함한 묶음 하나이며 사실 JSON을 고정했다. 입력 무삭제는 간섭 부재의 증거가 아니다. S4의 첫 실패와 v1.1 완료를 분리하고 모델 품질·운영 승격을 자동 승인하지 않는다. 최신 v1.20은 [Claude Code 코드 검토](../../history/2026-09-17-claude-product-review.md#claude-review)의 일상 어휘 오인·정의 요청 오차단·복합 요청 누락을 보완한 R16 단독 후보다. v1.18/v1.19와 S5 증거를 보존하며 실제 모델 품질은 아직 측정하지 않았다.
...
- 2026-09-17 Claude Code 오케스트레이션: [코드 검토·보완 기록](../../history/2026-09-17-claude-product-review.md#claude-review)에 실제 CLI 코드 검토·로컬 재현·새 v1.20 회귀·부모 재검증을 연결했다. Phase 10 유지보수이며 새 Phase·학습·운영 전환·S6 실행은 추가하지 않는다. 잔여 49요청은 그대로다.
...
- 2026-09-16 S4 완료: [복구·비교 기록](../../history/2026-09-16-phase9-s4-recovery.md#phase9)의 192요청·전량 검증·후보 미채택을 반영했다. 잔여는 실패 1건을 포함해 49이며 다음은 Phase 10 최소 후보다. 실제 후속 구현·데이터·학습·운영 변경은 하지 않았다.

- 2026-09-16 S4 중단: 모델 수집·실제 dry-run 뒤 첫 K0 요청의 저장 계약 오류로 중단했다. [실행 기록](../../history/2026-09-16-phase9-s4-execution.md)에 실패 증거·CPU 대조·1오류/191미실행·잔여 241을 연결했다. Phase 9 완료·모델 품질 판정·재실행을 자동 승인하지 않는다.

- 2026-09-16 S4 구현: 공식 모델 등록·실행기·CPU 검증을 [별도 기록](../../history/2026-09-16-phase9-s4.md)으로 연결했다. 실제 비교·Phase 9 완료와 구분하며 새 모델 다운로드·GPU 생성·운영 변경은 하지 않았다.

- 2026-09-16 S3 완료: 96요청·공개 build 재구성 검증과 원응답 한계 대조를 완료했다. 실제 과제 M-0292·O-0172에 8B 완료 증거를 연결했으며 원문·588행의 문서 반영 상태는 그대로다. 잔여 242요청과 다음 Phase 9를 명시하고 운영·학습은 변경하지 않았다. [실행 기록](../../history/2026-09-16-phase8-intent-s3.md#phase8b)을 따른다.

- 2026-09-16: 8A v1.17 후보·의도 v2를 구현하고 새/부모 CPU·화면 회귀를 통과했다. 원문 588행 중 실제 과제 O-0171만 실행 완료 증거를 연결했다. 8B·운영 배포는 미완료이며 [실행 기록](../../history/2026-09-16-phase8-intent-s3.md)을 따른다.

### 2026-09-15 — Phase 7 보완 정본화

- 정본 권한·반영표의 유형/문서/실행 상태·재귀 검사 범위를 보완한다. Phase 8A/10의 경계, 11→12→13 결정, 후보 전체 구성 동결, 잔여 예산과 운영 기준선·되돌림을 기존 절에 반영한다. 이번 보완의 검증·완료 상태는 [진행 기록](../../history/2026-09-15-phase7-canonicalization.md#supplement-20260915)을 따른다.

### 2026-09-15 — Phase 7~14 정본화

- 검토 원문 3개를 byte 보존하고 588행의 전량 반영표와 8개 Phase 문서로 연결했다. Phase 7 문서·정합성 검증을 완료했고 관련 32개·전체 947개 테스트를 통과했다. 후속 구현·생성·학습·서비스 전환은 미실행이다.
- 검증 명령·결과·보존 확인은 [정본화 기록](../../history/2026-09-15-phase7-canonicalization.md)에 남긴다. 아래 날짜별 기록은 당시 이력으로 유지한다.

### 2026-09-15 — 검사 보강·CPU 파생·앱 의도 후보 완료

- 파생 `build-8547c487c858`와 v1.16 CPU canary `build-641ac655f656`를 연결했다. 원문·차단·모델·학습·승인·운영은 보존했다. 다음 별도 실행 축을 S3로 옮겼으며 전체 S0~S6 완료로 표시하지 않는다. 검증·미실행 경계는 실행 정본의 최신 진행 기록을 따른다.

### 2026-09-15 — S2 실행 완료·검사 보강을 다음 최소 작업으로 지정

- S0/S1 계약 검증과 S2 342요청·재검증을 반영했다. 실제 모델 오류와 검사기 오탐을 구분하며 원래 집계를 보존한다. [완료 기록](../../history/2026-09-15-system-context-diagnosis.md)에 조건별 분모·제한·검증을 고정했다.
- 전체 ML 테스트 879건 통과. S3~S6·60/70·서비스 전환·승격은 실행하지 않았고 전체 A~D 진단 완료로 표시하지 않는다.

### 2026-09-14 — 기본 브랜치 통합 완료

- v1.15·20문장 진단·실행 정본·분기 정책을 PR #28로 통합하고 원본 `master`에서 후속 상태를 정리한다. 코드 병합과 운영 배포를 구분하며 기존 서비스·모델·자료는 유지한다.
- 테스트·설정 검증과 정리 결과는 [통합 기록](../../history/2026-09-14-default-branch-integration.md)을 따른다. S0~S6 진단·학습·승격은 실행하지 않았다.

### 2026-09-14 — 전체 경로와 컨텍스트 실험 설계 연결

- 원인 분리 순서를 계산/상태/전달부터 확인하는 S0~S6로 구체화했다. 50-B의 정보량·지시문 비교와 50-C의 크기×정보 교차 비교를 분리했다. 실행 상세는 [새 계획](../saju_system_context_diagnosis.md)에 위임하고 기존 Phase·승인 경계는 유지한다.
- 변경은 문서·정합성 테스트뿐이다. 검증 명령·결과와 미실행 범위는 새 계획의 진행 기록을 따른다. 모델 실행·학습·운영 전환·병합은 하지 않았다.
...
### 2026-09-02 — 단일 날짜 Relation v1 승인

- `KR_NATAL_DAY_RELATIONS_V1` 정책에서 기간 연·월·일 십신, 기간 일지와 원국 네 지지의 합·충·형·파·해 존재, 동일 간·지 반복을 해석과 분리했다.
- 자동 conformance `build-aa13a1333586`은 천간 100·지지 본기 120·지지쌍 144 전수에서 mismatch 0이며, 범위 부모·변조 부모·release 누락 수락도 0이다.
- `saju-natal-day-relation-release-v1.0.0-554bb9bfaea9`을 feature 기본 off로 발행했다. 이 체크포인트에서는 dashboard v1.13 연결, strict/full, sealed blind, MIX20K-v3.1, 학습·승격 권한을 열지 않았다.

### 2026-09-02 — Dashboard v1.13 relation 후보 검증 완료

- v1.12를 부모로 하는 v1.13 config·assets·진입점과 relation session binding을 별도 버전으로 추가했다.
- 단일 날짜에만 relation card와 model allowlist를 열고, 범위에는 공식 날짜별 label과 `relation: null`만 유지한다. 원국·기간·관계 부모 hash가 다르거나 내부 ID가 섞이면 fail-closed한다.
- 자동 canary `build-eaeeb35866d1`은 8개 stratum 160/160을 통과했고 실제 암호화 session을 새 process에서 20회 복원했다.
- 비활성 status의 복사 잔재를 찾아 schema `1.3.0`으로 교정하고 자동 회귀 검사를 추가했다. relation·dashboard·기간 표적 unittest 94건과 전체 Ruff가 통과했다.
- 전체 회귀 점검에서 남아 있던 비자동 Gate 금칙 용어를 운영 기록의 사실 의미를 유지한 채 자동 진단 용어로 교정했다.
- feature 기본 off와 dashboard v1.11 운영 상태를 유지했다. GPU 생성·sealed blind·MIX20K-v3.1·학습·모델 승격·Phase 6 판정은 변경하지 않았다.
