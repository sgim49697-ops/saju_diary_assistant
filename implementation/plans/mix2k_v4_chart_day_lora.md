...
- S3→S4→S5와 행동 7축·400건 선택·학습 출발점 결정을 새 Phase에 연결했다. 승인 데이터·adapter·teacher 기록을 변경하지 않았으며 검증은 [정본화 기록](../history/2026-09-15-phase7-canonicalization.md)을 따른다.
...
- 운영 연결: PR #24를 master merge commit `21dde6def0a9fa8c55f260765cadf82fabb60434`로 병합했다. `saju-mix2k-r16-dashboard-v1-14.service`는 `127.0.0.1:8767`에서 active·restart 0이며 root/status/runtime/session/dataset HTTP smoke와 암호화 원국→2026-09-04 단일 일진→즉시 세션 삭제 흐름을 통과했다. 실제 생성 canary 세션은 원문 보존용 숨김 파일로 이동해 일반 세션 목록에는 노출하지 않았다.
- R8/R32 시작: 기존 PATH 실패 unit은 이력으로 보존하고 새 `saju-mix2k-lora-r8-r32-retry-20260904.service`를 시작했다. unit-local PATH에 `/usr/lib/wsl/lib`를 명시하고 trainer SHA `7274abaa3f4750ab1fc2d266980fcf18ff80e02c5d14c45688bd7bb33bfc6a81`와 config SHA `d7c5db056be927319617ac4b932acb9e37d9f9a2e6478598d20f2b7ce12fa728`를 실행 직전에 검증한다. 완료 R16은 `mode=reused`로 검증됐고 R8 `preflight-9e17ed2bebca` 실행 진입, 이후 R8 1 epoch `train-7ce05b84b1aa`→R32 `preflight-68a6f3befac6`→1 epoch `train-0a3ada2de1db` 순서다. 시작 시 unit active와 dashboard HTTP 200을 확인했으며 사용자 지시대로 이 세션에서는 완료를 계속 추적하지 않는다.
