...
## 현재 위치와 후속 정본 — 2026-09-14

이 문서의 구현과 20문장 60요청·54생성·6차단 진단은 완료됐다. 후보 commit `2646213`과 후속 정본은 PR #28 `3bb0ce2`로 `master`에 병합됐으며 원본 프로젝트 폴더도 동기화했다. v1.15는 병합 완료·운영 미배포 상태이고 기존 dashboard v1.14 서비스는 유지한다. 아래 실행 순서와 날짜별 기록은 완료 범위의 재현 이력이지 자동 재실행 지시가 아니다.

후속 실행 순서는 [로드맵 README](saju_product_roadmap/README.md), 현재 상태는 [00 기준선](saju_product_roadmap/00-current-baseline.md), 진단 상세는 [50 원인 분리](saju_product_roadmap/50-automatic-model-evaluation.md)가 소유한다. 시스템 지시문 후보 비교·큰 기본 모델 비교·입력/데이터 점검은 별도 후속이며 이번에 실행하지 않는다.
...
### 2026-09-14 — 후보 코드 병합과 운영 경계 유지

- PR #28로 후보 코드와 공개 진단을 기본 브랜치에 통합했다. GPU 재진단·운영 배포·서비스 재시작은 하지 않았다. 통합 검증과 후속 폴더 정리는 [통합 기록](../history/2026-09-14-default-branch-integration.md)을 따른다.

### 2026-09-05 — 완료 후보에서 원인 분리 정본으로 인계

- 기존 v1.15 코드·지시문·20문장·공개 report는 변경하지 않고 후속 50-A~D 연결만 추가했다. 큰 기본 모델 비교는 필수 후속이지만 아직 등록·실행하지 않았다.
- 검증 명령·결과는 [재정렬 기록](../history/2026-09-05-model-cause-roadmap.md)을 따른다. 운영 service·기본 모델·학습·release·Phase 6은 유지한다.
...
### 2026-09-05 — 확정 20문장 모델 테스트 완료

- 실행 코드 `f973f04`에서 세 모델 60요청·54생성·6예정 차단을 완료했다. 첫 질문 12개 그룹의 token identity 일치, 54응답 입력 재구성과 부모 이력 60요청 대조, 재개 추가 생성 0회·산출물 불변을 확인했다.
- 기존 연결 구조 검사 통과는 K0 8/13, R16 10/13, KI20 8/13이다. 세 모델 모두 잘못된 일간 전제를 교정하지 못했다. 최대 입력은 2,024 token·제외 대화 0이며 context 부족의 근거는 없다. 유한 검사기의 오탐·누락과 모델의 오답은 분리하고 의미 품질 승격으로 해석하지 않는다.
- 공개 build는 `data/reports/saju_1b_baseline/dashboard-prompt20/v1.0.0/build-9ab2958c83dc`이다. 원출력은 Git 제외 새 진단 경로에만 보존했다. 모델별 실제 관찰과 다음 조정 순서는 [완료 기록](../history/2026-09-05-dashboard-prompt20.md)에 정리했다.
- 이번 요청은 현재 지시문에서의 모델 테스트까지 완료했다. 지시문 수정·추가 학습·모델 승격·Phase 6·release·서비스 전환은 수행하지 않았다. 기존 서비스는 동일 process에서 HTTP 200을 유지했다.
- 마감 검증: 공개 JSON 3개의 원문 field 비노출과 manifest·artifact·실행 코드 SHA, private 집계 일치, 문서 링크 7개를 확인했다. 최종 문서 정책 5건·Ruff·공백 검사도 통과했다.
