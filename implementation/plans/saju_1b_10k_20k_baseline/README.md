...
Phase 0~6의 상태·아래 고정 Full FT 계약·날짜별 기록은 완료 이력으로 유지한다. 이후 완료된 기간·관계 Runtime, R8·R16·R32 LoRA, 20문장 진단을 이 초기 실험의 미완료 단계로 되돌리지 않는다. 현재 앱은 v1.14 운영 / 최신 v1.17 CPU·합성 화면 검증 후보·master 통합·운영 미배포이며, v1.16 부모와 아래 v1.11 당시 운영 이력은 보존한다. 코드 통합과 기본 브랜치 동기화는 기존 Phase 상태를 변경하지 않는다.
