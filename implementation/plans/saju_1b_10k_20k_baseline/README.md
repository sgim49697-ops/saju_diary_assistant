...
## 완료 이후 후속 정본 — 2026-09-16

Phase 0~6의 상태·아래 고정 Full FT 계약·날짜별 기록은 완료 이력으로 유지한다. 이후 완료된 기간·관계 Runtime, R8·R16·R32 LoRA, 20문장 진단을 이 초기 실험의 미완료 단계로 되돌리지 않는다. 현재 앱은 v1.14 운영 / 최신 v1.18 R16 제품 후보 CPU·합성 화면 검증 완료·운영 미배포이며, v1.16·v1.17 부모와 아래 v1.11 당시 운영 이력은 보존한다. 실제 모델 호출 없이 Phase 10 구현을 마쳤으며 다음은 Phase 11의 학습 가설·조건부 명세다. 코드 통합과 기본 브랜치 동기화는 기존 Phase 상태를 변경하지 않는다.
...
새 ZIP 3개는 [원문 반영표](../saju_product_roadmap/source-20260915.md)로 보존하고 번호를 Phase 7~14로 이어간다. Phase 7 정본화와 Phase 8A 후보 검증·8B S3 96요청을 완료했고 다음 별도 과제는 Phase 9/S4다. [실행 기록](../../history/2026-09-16-phase8-intent-s3.md)의 P1 미채택·검사 한계를 따른다. Phase 13 학습·14 운영 전환은 조건부이며 이 초기 실험의 고정 계약을 자동으로 재실행하지 않는다. 아래 `4.0.5`·기준일·Phase 0~6 표는 완료 계약의 식별자다.
...
- 완료 계약·상태는 보존하고 새 문서의 실행 순서와 운영/후보 구분만 갱신했다. 검증과 후속 범위는 [정본화 기록](../../history/2026-09-15-phase7-canonicalization.md)을 따른다.
