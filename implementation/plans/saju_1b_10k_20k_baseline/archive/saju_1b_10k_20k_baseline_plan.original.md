...
- 데이터 카드의 품질 주장은 제작자 자체 설명이므로, 검산된 Gold로 취급하지 않는다.
...
8. 공감 대화의 스타일 재작성
9. 소스별 품질 가중치
10. 검산되지 않은 해석 Gold oversampling
...
| 소스 | v0 정답 | 라벨 종류 | 의미 |
|---|---|---|---|
| Nemotron | 원본 4키 서사 JSON | `auto_validated_synthetic` | 구조 검사는 됐지만 결정론 통변 Gold는 아님 |
...
# 9. 10K 이후 자동 정답 라벨 개선 절차

10K 모델 결과를 본 뒤 전체 데이터를 다시 라벨링하지 않는다. **오류가 많은 구간부터 소규모 자동 보정 세트를 만든다.**
...
```text
처분: ACCEPT / REGENERATE / REJECT
구조·schema 일치: true / false
입력 사실 위반 수: integer
결정론 규칙 위반 수: integer
언어 문자 비율: ko / zh / en
과도한 단정 패턴 수: integer
답변 반복률: float
정책 밖 의미 품질: not_measured
재생성 계약 ID: 선택 입력
```

`REGENERATE`는 고정 규칙으로 재생성하고 같은 자동 계약을 다시 통과한 경우에만 `A2`가 된다. `REJECT`는 이후 manifest에서 제외한다.
...
고정 규칙·결정론 계산 검증 없이 모델 출력을 다시 학습 정답으로 넣으면, 모델의 초기 오류를 반복 강화할 수 있다.
...
1. `REJECT` 판정 행은 제외하고, 보충할 경우 `old_id → replacement_id`를 기록
2. `A2` 자동 재생성 정답 적용
...
각 모델에서 동일 100문항을 같은 deterministic grader로 비교한다.
...
자동 축은 입력 사실 위반, 필수 action, JSON/schema, 언어 문자 비율, 반복률, 금지 표현, 상태 모순으로 고정한다. 계약 밖 자연스러움과 의미 품질은 `not_measured`로 기록하고 Gate나 후속 작업으로 요구하지 않는다.
...
## Step 9. 오류 분류 및 400개 자동 보정

ACCEPT / REGENERATE / REJECT 및 자동 기술 라벨을 작성한다.
