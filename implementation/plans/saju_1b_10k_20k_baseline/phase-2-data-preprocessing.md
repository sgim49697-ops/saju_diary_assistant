...
| 출력 | unified v0, holdout/core eval, 소스별 후보 순서, 감사 보고서 |
| 완료 Gate | 스키마·누수·중복·후보 여유분 검사 통과 |
| 웹 확인일 | 2026-08-27 |

## 목적

소스별 원문을 재현 가능한 공통 대화 스키마로 변환하고, 평가 누수를 먼저 차단한 뒤 10K·20K baseline manifest를 만든다. v0에서는 학습을 망가뜨리는 오류만 제거하며 문체·명리 해석을 대규모로 다시 쓰지 않는다.
...
`id`, `source_revision`, `license`, `task`, `raw_hash`, `group_id`는 null을 허용하지 않는다. 원본 ID가 없으면 source 파일 SHA-256, 행 번호, 정규화 전 원문 SHA-256으로 결정론적 ID를 만든다.
...
| `open_saju_qa` | 일반 사주 상담·Q&A |
| `pillar_calculation` | 날짜에서 네 기둥 출력 |
...
| YEJI v9 | `synthetic_soft_gt` |
| YEJI Processed | `source_soft_gt` |
| YEJI Translated | `translated_weak_gt` |
| AI Hub 감성대화 | `human_reference` |
| AI Hub 연속대화 | `human_next_turn` |

라벨 상태는 `R0`, `A1`, `A2`, `H1`, `H2`, `D`만 쓴다. v0 학습 후보는 원칙적으로 `R0+A1`이며 가중치를 차등 적용하지 않는다.
...
- Alpaca 한 형식의 `domain == "bazi"`만 사용한다.
- `instruction`과 선택적 `input`을 user에, `output`을 assistant에 둔다.
- 답변 token 길이 사분위별 후보를 유지해 장문 쏠림을 막는다.
- 정규화 instruction hash와 템플릿 fingerprint를 `group_id`로 사용한다.

### YEJI Processed

- 실제 파일 스키마에서 instruction/output 대응 필드를 adapter mapping으로 명시한다.
- `domain`에 `bazi`가 포함되고 입력·출력이 존재하는 행만 사용한다.
- 출력이 완전히 중국어면 제외한다.
- 정규화 instruction hash를 `group_id`로 사용한다.

### YEJI Translated

- `question_ko`만 user, `answer_ko`만 assistant에 사용한다.
- assistant가 정확히 네 간지 쌍으로 파싱되는 행만 남긴다.
- `公历`, `左右` 등 중국어 문장 잔재가 심하면 재작성하지 않고 제외한다.
- 생년월일시, 정답 네 기둥, 질문 템플릿 fingerprint를 `group_id`로 사용한다.
...
각 소스에서 group 단위로 100개씩 먼저 고정한다. 동일 group의 다른 행은 학습 후보에서 모두 제외한다.
...
2. 아래 MIX10·MIX20 수량표와 `MIX10 ⊂ MIX20` 계약을 machine-readable spec으로 저장한다.
3. Phase 4가 고정 tokenizer로 512/768/1024 token 적격 행을 계산한다.
4. Phase 4는 각 길이에서 후보 순서대로 MIX20 수량을 채우고, source별 앞 절반을 MIX10으로 고정한다.
5. 수량·포함 관계 검증 후 Phase 4가 각 manifest 행 순서를 seed 42로 별도 shuffle한다.

| 소스 | MIX10 | MIX20 |
|---|---:|---:|
| Nemotron | 3,500 | 7,000 |
| YEJI v9 | 2,000 | 4,000 |
| YEJI Processed | 2,500 | 5,000 |
| YEJI Translated | 500 | 1,000 |
| AI Hub 감성대화 | 1,000 | 2,000 |
| AI Hub 연속대화 | 500 | 1,000 |
...
- [ ] 후보 순서와 MIX10⊂MIX20 수량 계약을 seed 42로 고정했다.
...
```text
data/unified/v0_raw/<source>.jsonl
data/manifests/candidates/source_candidate_order_v0_nc.jsonl
data/manifests/candidates/mix_contract_v0_nc.json
data/eval/source_holdout_600.jsonl
