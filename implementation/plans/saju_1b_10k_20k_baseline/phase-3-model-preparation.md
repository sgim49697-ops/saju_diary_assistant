...
| 출력 | `.venv`, requirements lock, 모델 snapshot, chat template 후보, 환경 보고서 |
...
| 용도 | 모델 | revision |
|---|---|---|
| 학습 시작점 | `kakaocorp/kanana-2-1.3b-base` | `e9ffedf7b713530ae6a0c94ea32538d75e8524e1` |
| 원본 비교·template 기준 | `kakaocorp/kanana-2-1.3b-instruct` | `bf4786aa2a1908adce942d53976270132732f720` |

Base는 `Kanana2TinyForCausalLM` custom hybrid attention 모델이며 공식 카드가 `trust_remote_code=True`와 `transformers >= 4.57`을 요구한다. 모델 config의 최대 context는 32,768이지만 baseline 학습 길이는 메모리 Gate가 고른 512/768/1024 중 하나로 제한한다.
...
1. `hf download --dry-run`으로 Base 약 2.6GB weights와 repository file 목록을 확인한다.
2. Base와 Instruct를 각각 고정 revision으로 private local model cache에 다운로드한다.
...
```bash
hf download kakaocorp/kanana-2-1.3b-base \
  --revision e9ffedf7b713530ae6a0c94ea32538d75e8524e1 \
  --dry-run
```

## Chat template 결정

Base tokenizer에는 chat template가 없지만 `<|im_start|>`, `<|im_end|>`, `<|begin_of_text|>`, `<|end_of_text|>` token이 존재한다. Instruct 저장소의 공식 template를 고정 후보로 사용한다.
...
template는 `configs/chat_templates/kanana2_sft.jinja`에 원문 그대로 고정할 예정이다. Phase 4에서 render 결과, special token, EOS 종료와 loss mask가 모두 맞아야 채택한다. 검증 실패 시 임의 수정하지 않고 Phase 3으로 돌아와 변경안과 새 hash를 기록한다.
...
Base/Instruct config·tokenizer 로드 성공
Base parameter count와 dtype 기록
...
- [ ] Base·Instruct snapshot과 remote code SHA-256을 기록했다.
- [ ] 두 모델의 config·tokenizer를 오프라인으로 다시 로드했다.
...
- [Kanana 2 1.3B Base](https://huggingface.co/kakaocorp/kanana-2-1.3b-base/tree/e9ffedf7b713530ae6a0c94ea32538d75e8524e1)
...
| 날짜 | 확인 내용 | 결과 |
|---|---|---|
| 2026-08-27 | Kanana 모델 카드·config | custom code, Transformers 4.57+, 32K context 확인 |
