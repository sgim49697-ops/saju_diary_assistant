...
- 날짜: 2026-08-26
- 작업 요약: v7 생성 데이터 확인용으로 `data/train-extra-00000.parquet` 한 개를 동일한 고정 리비전에서 추가 다운로드했다.
- 변경 범위: 로컬 원본 129,879,649 bytes를 추가해 v6 66,666행과 v7 50,000행, 총 116,666행을 확보했다.
- 검증 결과:
  - 로컬 SHA-256 `97616a1ae8725c68a665e9aef5396988cf16acfce1cc271a2c209c2b671d687a`가 Hugging Face LFS OID와 일치했다.
  - v6·v7 모두 40개 컬럼으로 스키마가 일치하고, 총 116,666개 UUID 중 중복이 없었다.
  - v7의 핵심 필드 누락, 주요 JSON 파싱 오류, 서사 생성 오류는 모두 0건이었다.
  - 평균 `saju_narrative` 길이는 v6 696.2자, v7 552.5자로 차이가 확인됐다.
- 남은 이슈/후속 작업: 두 shard로 버전 간 비교는 가능하지만, 전체 모집단 추정이 필요하면 나머지 shard에서 소량 층화 표본을 추가한다.
