...
다만 Nemotron은 현대 한국인 페르소나 데이터셋이고 직업·경력 필드에 영문 표기가 흔하다. 이 필터는 무작위 손실이 아니라 IT·디자인·미디어·경영 직군 페르소나를 선택적으로 제거한다. 소견 2 때문에 제거 규모를 확인할 수 없다.
...
## 소견 5 (중) - 단일턴 축이 실제로는 멀티턴 대화의 절단본
...
두 축의 구분은 데이터 속성이 아니라 `ordered_groups[:single_target]`과 `[single_target:required]`라는 rank 위치다. 그룹 비중첩 계약(`aihub_cross_axis_group_overlap: 0`)은 충족되지만, 정본이 기술한 "단일턴 축"의 의미와는 다르다.
