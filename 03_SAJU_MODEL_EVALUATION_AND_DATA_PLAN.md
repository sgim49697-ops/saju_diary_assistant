...
| 최신 전체 경로 진단 | S0/S1 검증·S2 342요청·312생성·30차단·재검증 완료 | 검사기 오탐 보강 제안, 지시문 A/B·큰 기본 모델 비교는 미실행 |
| MIX20K-v3.0.1 | 보정·비학습 후보 이력 | 현재 2K 학습 데이터와 별도, v3.1 생성 승인 아님 |

## 최근 진단이 말해 주는 것

[S2 완료 기록](implementation/history/2026-09-15-system-context-diagnosis.md)에서 최대 입력 1,692 token·삭제 이력 0·전체 테스트 879건 통과를 확인했다. 다만 R16 자동 회귀 8개 중 6개가 부정·과거 인용·label 추출 오탐이었다. 자동 통과 수를 실제 정확도나 모델 순위로 읽지 않는다. 실제 모델 오류와 앱 의도 분류 오류도 각각 확인했으며, 다음 최소 제안은 새 검사 버전으로 기존 합성 응답의 파생 집계를 만드는 것이다. 새 GPU 생성이나 재학습이 필요한 작업은 아니다.
