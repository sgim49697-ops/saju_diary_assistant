# saju_runtime_v1_5.py - 원국+단일 일진 release runtime v1.5 CLI를 제공한다.
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="한국 만세력 원국+단일 일진 runtime v1.5")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "verify-contract":
            ...
            release = validate_release_registry_v1_5(args.release_registry)
            result = {
                "status": "verified_chart_and_single_day",
                "release_id": release["release_id"],
                "approved_tools": release["approved_tools"],
                "approved_single_day_range": release["approved_single_day_range"],
