# dashboard_product_canary.py - Phase 10의 CPU·합성 화면 검증만 불변 공개 집계로 발행한다.
...
PUBLIC_ROOT = REPO_ROOT / "data/reports/saju_1b_baseline/dashboard-product-canary/v1.0.0"
CODE = (
    "scripts/training/phase5_dashboard_v1_18.py",
    "scripts/training/dashboard_product_policy_v1.py",
    "scripts/training/dashboard_product_session_v1.py",
    ...
    "configs/model_versions/saju_1b_baseline/phase5-dashboard-v1.18.0-product-candidate.json",
    "configs/chat_prompts/saju_product_v1.txt",
    "scripts/evaluation/dashboard_product_canary.py",
    "scripts/evaluation/dashboard_product_ui_canary.cjs",
    "tests/test_dashboard_product_v1.py",
    "tests/test_dashboard_product_http.py",
    "tests/test_dashboard_product_canary.py",
    *("scripts/training/phase5_dashboard_assets/v1.18.0/" + name for name in ("dashboard.js", "dashboard.css", "index.html", "prompt-examples.json")),
)
PARENTS = {
...
TESTS = ("tests.test_dashboard_product_v1", "tests.test_dashboard_product_http", "tests.test_dashboard_product_canary")
TEST_COUNT = 46
...
def policy_cases():
    from tests.test_dashboard_product_v1 import binding_fixture, chart_binding
...
def identity():
    ...
    return {
        "canary_id": "dashboard-product-cpu-v1.0.0", "dashboard_version": "1.18.0",
...
def run_browser():
    result = subprocess.run(["node", str(REPO_ROOT / "scripts/evaluation/dashboard_product_ui_canary.cjs")], cwd=REPO_ROOT, env={**os.environ, "CUDA_VISIBLE_DEVICES": ""}, capture_output=True, text=True, timeout=90, check=False)
    if result.returncode:
        raise ValueError("제품 합성 화면 검사 실패")
    value = json.loads(result.stdout)
    if value.get("passed") is not True or value.get("cases_passed") != 16 or value.get("model_generations") != 0 or value.get("production_service_accessed") is not False:
...
def evaluate():
    ...
    aggregate = {
        ...
        "dashboard_version": "1.18.0", "candidate_engine": "lora_r16",
...
def main(argv=None):
    ...
    if args.command in {"validate-contract", "plan"}:
        frozen = identity()
        result = {"build_id": "build-" + policy.digest(frozen)[:12], "status": "validated" if args.command == "validate-contract" else "dry_run", "tests_planned": TEST_COUNT, "browser_cases_planned": 16, "writes_performed": False, "governance": GOVERNANCE}
