# system_context_scoring_v1_2.py - S4 전에 부정 범위와 일반 답변의 원국 삽입 회귀를 고정한다.
...
SCORER_VERSION = "role-aware-contract-v1.2.0"
...
DENIAL = re.compile(
    r"(?:이|가|은|는)?\s*(?:결코\s*|전혀\s*|절대\s*)?(?:(?:사실|전체|자체|전부)(?:이|가|은|는)?\s*)?(?:아니|아닙|아닌|틀|잘못)"
    r"|(?:을|를)?\s*(?:새로\s*)?(?:붙이|덧붙이|주장하|단정하)(?:지|지는)\s*않"
...
def unsolicited_reference(output):
    """일반 작업에 원국 사실을 섞는 경우와 명시적 미사용 안내를 구분한다."""
    for sentence in SENTENCE.finditer(output):
        for match in re.finditer(r"(?:원국|명식|사주)(?:의)?\s*(?:사실|정보|구조|흐름)", sentence.group()):
            tail = sentence.group()[match.end():]
            # 별도 절의 부정이 앞선 사주 삽입을 면책하지 않게 국소 범위를 제한한다.
            clause = re.split(r"[,;]|지만|는데", tail, maxsplit=1)[0]
            if not re.search(r"(?:쓰|사용하|섞|넣|참고하|언급하)(?:지|지는)\s*않|제외하|빼겠", clause[:48]):
                return True
    return False
...
def score(case, output, *, stop_reason="eos"):
    ...
    if case["task"] in {"general", "two_sentences"}:
        unsolicited = asserted or unsolicited_reference(output) or re.search(
