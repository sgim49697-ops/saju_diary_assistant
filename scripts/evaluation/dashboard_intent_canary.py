# dashboard_intent_canary.py - v1.16의 CPU 합성 경로 검사만 실행하고 불변 공개 집계를 발행한다.
...
PUBLIC_ROOT = REPO_ROOT / "data/reports/saju_1b_baseline/dashboard-intent-canary/v1.0.0"
CODE = (
    "scripts/evaluation/dashboard_intent_canary.py",
    "scripts/evaluation/system_context_rescore.py",
    "scripts/training/phase5_dashboard_v1_16.py",
    "scripts/training/dashboard_grounding_v3.py",
    "configs/model_versions/saju_1b_baseline/phase5-dashboard-v1.16.0-intent-candidate.json",
    "tests/test_dashboard_grounding_v3.py",
    "tests/test_phase5_dashboard_v1_16.py",
    "tests/test_dashboard_intent_canary.py",
)
TESTS = (
...
def identity():
    ...
    return {
        "canary_id": "dashboard-intent-cpu-v1.0.0",
        "parent_manifest_sha256": PARENT_PINS["build_manifest.json"],
        "source_sha256": {**source, **{p: file_sha(REPO_ROOT / p) for p in CODE}},
...
def evaluate():
    ...
    summary = {
        ...
        "parent_date_extraction_unchanged": True,
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="v1.16 CPU 합성 canary; 모델 생성·서비스 변경 없음"
    )
