# release_registry.py - 통과한 conformance v3를 v1.1 runtime release로 원자적으로 승인한다.
...
RELEASE_ROOT = REPO_ROOT / "configs/runtime/calculation/releases/v1.1.0"
...
def _relative(path: Path) -> str:
    try:
        ...
        raise RuntimeCalculationError(
            "RUNTIME_RELEASE_INVALID", "release 입력이 저장소 밖에 있습니다."
        ) from exc
...
def build_release(report_path: Path) -> dict[str, Any]:
    ...
    report, _manifest = _validate_report_identity(identity)
    ...
    if not isinstance(implementations, dict) or not isinstance(official, dict):
        raise RuntimeCalculationError(
            "RUNTIME_RELEASE_INVALID", "conformance 입력 identity가 비었습니다."
        )
    return {
...
def write_release(value: dict[str, Any], output: Path) -> Path:
    ...
    try:
        ...
        raise RuntimeCalculationError(
            "UNSAFE_CONTRACT_PATH", f"release는 {RELEASE_ROOT} 아래에만 쓸 수 있습니다."
        ) from exc
    if resolved != DEFAULT_OUTPUT.resolve(strict=False):
        raise RuntimeCalculationError(
            "UNSAFE_CONTRACT_PATH", "v1.1 release 파일명은 고정 경로여야 합니다."
        )
    if output.exists() or output.is_symlink():
        raise RuntimeCalculationError(
            "RUNTIME_RELEASE_EXISTS", "기존 release registry를 덮어쓰지 않습니다."
        )
    ...
    try:
        ...
        if descriptor >= 0:
            os.close(descriptor)
    validate_release_registry(output)
    return output


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="saju runtime v1.1 release 승인기")
