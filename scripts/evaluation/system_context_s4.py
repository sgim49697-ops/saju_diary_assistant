# system_context_s4.py - 공통 P0에서 K0·3B/FULL·MIN 192요청을 동결 실행·재개·검증한다.
...
def schedule(config, rendered=None):
    specs = cases()
    result = [
        {"stage": "primary", "engine": engine, "case_id": c["case_id"], "arm": arm}
        for c in specs for arm in config["context_arms"] for engine in config["engines"]
    ]
    if rendered is not None:
        eligible = {c["case_id"] for c in specs if not c["expected_block"]}
        first = [
            max(
                (r for r in result if r["engine"] == engine and r["case_id"] in eligible),
                key=lambda r: (rendered[(r["case_id"], r["arm"], engine)]["input_tokens"], r["case_id"], r["arm"]),
            )
            for engine in ENGINES
        ]
        result = [*first, *[r for r in result if r not in first]]
    for index, entry in enumerate(result, 1):
        entry["request_id"] = f"request-{index:03d}"
    if len(result) != 192 or len(result) > config["maximum_requests"]:
        raise ValueError("S4 192요청 예산 위반")
    return result


def _stable_case(case):
    # S4는 부모의 capability까지 그대로 동결한다. 재계산·신규 binding은 만들지 않는다.
...
def prepare():
    ...
    selected = {
        ...
        if r["stage"] == "primary" and r["engine"] == "k0_instruct" and r["arm"] in config["context_arms"]
    }
    expected_keys = {(c["case_id"], arm) for c in cases() for arm in config["context_arms"]}
    if set(selected) != expected_keys:
        raise ValueError("동결 부모 48개/FULL·MIN이 다릅니다.")
    context = prepare_context()
    model_files = verify_models(context)
    registry = registry_summary(context, model_files)
    tokenizers = {e: load_tokenizer(e) for e in ENGINES}
    rendered, comparisons = {}, []
    for spec in cases():
        case_id = spec["case_id"]
        full, minimum = selected[(case_id, "C_FULL")], selected[(case_id, "C_MIN")]
        if full["case"] != minimum["case"] or full["parent_sha256"] != minimum["parent_sha256"]:
            raise ValueError("S4 정보 조건의 상태/부모 이력이 다릅니다.")
        for arm in config["context_arms"]:
            original = selected[(case_id, arm)]
            rebuilt = render(original["case"], arm, context, tokenizers["k0_instruct"])
            if rebuilt != original["render"]:
                raise ValueError("S4 P0 정보 선택이 S2 동결 원문과 다릅니다.")
            for engine in ENGINES:
                full_tokens = rendered.get((case_id, "C_FULL", engine), {}).get("input_tokens")
                current = retokenize(
                ...
                rendered[(case_id, arm, engine)] = current
            comparisons.append(compare_inputs(
                rendered[(case_id, arm, "k0_instruct")], rendered[(case_id, arm, "kanana3b_instruct")],
                case_id=case_id, arm=arm,
            ))
    requests = [
        {
            **entry, "case": deepcopy(selected[(entry["case_id"], entry["arm"])]["case"]),
            "render": rendered[(entry["case_id"], entry["arm"], entry["engine"])],
            "parent_sha256": selected[(entry["case_id"], entry["arm"])]["parent_sha256"],
        }
    ...
    identity = {
        ...
        "cases_sha256": digest([selected[(c["case_id"], "C_FULL")]["case"] for c in cases()]),
        "input_sha256": digest(requests), "input_comparison_sha256": digest(comparisons),
        "model_registry_sha256": digest(registry), "verified_model_files": model_files,
...
def validate_prepared(value):
    ...
    for request in requests:
        key = (request["engine"], request["arm"])
        rows = groups.setdefault(request["case_id"], {})
        if key in rows or request["stage"] != "primary" or request["parent_sha256"] != digest(request["case"]["history"]):
            raise ValueError("S4 중복/단계/부모 불일치")
        rows[key] = request
    for rows in groups.values():
        if set(rows) != {(e, a) for e in ENGINES for a in ("C_FULL", "C_MIN")}:
            raise ValueError("S4 2×2 대응쌍 누락")
        reference = next(iter(rows.values()))
        if any(r["case"] != reference["case"] or r["parent_sha256"] != reference["parent_sha256"] for r in rows.values()):
            raise ValueError("S4 동결 상태/이력 불일치")
        for arm in ("C_FULL", "C_MIN"):
            if rows[("k0_instruct", arm)]["render"]["messages"] != rows[("kanana3b_instruct", arm)]["render"]["messages"]:
                raise ValueError("S4 두 모델의 입력 의미 불일치")
    if {r["engine"] for r in requests[:2]} != set(ENGINES) or any(r["case"]["expected_block"] for r in requests[:2]):
        raise ValueError("S4 두 모델 최초 적격성 요청 불일치")
...
def validate_response(request, response):
    ...
    if status == "generated":
        from scripts.evaluation.system_context_s4_backend import generation_kwargs

        telemetry = response["telemetry"]
        if (
            telemetry.get("engine") != request["engine"]
            or telemetry.get("adapter_used") is not False
        ...
        if (
            consumers.get("model_engine") != request["engine"]
            ...
            with os.fdopen(log_fd, "wb") as logfile:
                process = subprocess.Popen(
                    [
                        ...
                        "-B",
                        "-m",
                        "scripts.evaluation.system_context_s4",
                        "worker",
                        "--input",
                        str(input_path),
                        "--output",
...
def public_manifest(prepared, entries, summary, publication):
    ...
    return {
        ...
        "input_comparisons": prepared["input_comparisons"],
        "k0_tokens_equal_s2": True,
        "p0_messages_equal_s2": True,
        "instruction_arm": "P0",
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="S4 K0·3B/FULL·MIN 동결 진단: dry-run 기본, 운영 교체 없음"
    )
    parser.add_argument(
        "command", choices=("validate-contract", "plan", "execute", "verify", "worker", "download", "verify-model")
    )
    ...
    try:
        ...
        if args.command == "download":
            result = download(execute=args.execute)
        elif args.command == "verify-model":
            result = {"status": "models_verified", "files": verify_models(prepare_context()), "gpu_used": False}
            ...
            result = {
                ...
                "primary": 192,
                "expected_preblocks": 20,
                "maximum_generations": 172,
                "preflight": 0,
