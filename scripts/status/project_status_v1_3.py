# project_status_v1_3.py - v1.2 renderer를 보존하며 대화 자동 진단을 포함한 v1.3 현황을 발행한다.
...
def validate_contract(config: Mapping[str, Any], repo_root: Path) -> dict[str, Any]:
    if (
        ...
        or config.get("status_version") != "v1.3.0"
        or config.get("canonical_plan_version") != "4.0.1"
        or config.get("dataset_name") != "saju_1b_baseline"
        or config.get("as_of") != "2026-09-01"
        or config.get("stage") != "phase6_completed_automatic_repair_required"
    ):
        raise ProjectStatusError("project status v1.3 identity가 다릅니다.")
    components = config.get("components")
    if not isinstance(components, list) or len(components) < 15:
        raise ProjectStatusError("project status v1.3 component가 부족합니다.")
    names: set[str] = set()
    for value in components:
        if (
            ...
            raise ProjectStatusError("project status v1.3 component 형식이 다릅니다.")
    ...
    if not required_components <= names:
        raise ProjectStatusError("project status v1.3 필수 component가 없습니다.")
    ...
    if (
        ...
        raise ProjectStatusError("project status v1.3 Phase 타임라인이 다릅니다.")
    axes = config.get("dataset_axes")
    if not isinstance(axes, list) or sum(int(value.get("rows", 0)) for value in axes) != 20_000:
        raise ProjectStatusError("project status v1.3 데이터 축 수량이 다릅니다.")
    if abs(sum(float(value.get("assistant_token_share_percent", 0)) for value in axes) - 100.0) > 0.001:
        raise ProjectStatusError("project status v1.3 token 비율 합계가 다릅니다.")
    ...
    if (
        ...
        or decision["runtime_release"] != "차단"
        or decision["mix20k_v3_training"] != "금지"
    ):
        raise ProjectStatusError("project status v1.3 결정 형식이 다릅니다.")
    for key in ("evidence_tiers", "gates", "known_risks", "web_sources", "validation_summary"):
        if not isinstance(config.get(key), list) or not config[key]:
            raise ProjectStatusError(f"project status v1.3 {key}가 없습니다.")
    if not any(value.get("name") == "Grounded dialogue 장문 자동 진단" for value in config["gates"]):
        raise ProjectStatusError("장문 자동 진단 Gate가 없습니다.")
    outputs = config.get("outputs")
    if outputs != {
        "root_html": "PROJECT_STATUS.html",
        "snapshot_root": "data/reports/saju_1b_baseline/project-status/"
        "v1.3.0/{build_id}",
    }:
        raise ProjectStatusError("project status v1.3 출력 경로가 다릅니다.")
    ...
    if config.get("implementation_files") != [
        ...
        "scripts/status/project_status_v1_3.py",
    ]:
        raise ProjectStatusError("project status v1.3 구현 fingerprint가 다릅니다.")
    return {"status": "valid", "status_version": "v1.3.0"}


def prepare_context(repo_root: Path, config_path: Path) -> dict[str, Any]:
    config = load_json(config_path, "project status v1.3 config")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="사주 일기 도우미 v1.3 현황 HTML")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "validate-contract":
            result = validate_contract(load_json(config_path, "project status v1.3 config"), REPO_ROOT)
        else:
            ...
            if args.command == "plan":
                ...
                result = build_status(context) if args.execute else {
                    "status": "dry_run",
                    "build_id": context["build_id"],
                    "writes_performed": False,
                }
