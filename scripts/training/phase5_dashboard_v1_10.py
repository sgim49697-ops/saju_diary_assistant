# phase5_dashboard_v1_10.py - 승인 원국 명시 연결과 자동 grounding Gate를 제공한다.
...
V110_ASSET_ROOT = ASSET_ROOT / "v1.10.0"
...
def _validate_chart_only_runtime(
    ...
    if (
        ...
        raise Phase5DashboardError(
            "Phase 5 dashboard chart-only runtime 계약이 다릅니다."
        )
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    elif schema_version in {"1.7.0", "1.8.0", "1.9.0", "1.10.0"}:
    ...
    elif schema_version in {"1.9.0", "1.10.0"}:
...
def _runtime_model_context_from_binding(
    ...
    prompt = (
        ...
        "전달합니다. 없는 사실은 추측하지 말고 신강약·격국·용신·미래 사건으로 "
        "확대하지 마세요.\n"
...
def execute_manual_generation(
    ...
    if selected_runtime_binding is not None:
        if (
            context["config"]["schema_version"] != "1.10.0"
            or selected_runtime_session_id is not None
        ):
            raise Phase5DashboardError(
                "chart-only model binding은 dashboard v1.10 전용입니다."
            )
        (
            runtime_context,
...
class DashboardRequestHandler(BaseHTTPRequestHandler):
    ...
    def _error(
        ...
        *,
        ...
        if self.server.context["config"]["schema_version"] == "1.10.0":
            value["code"] = code
        payload = json.dumps(
            value,
            ensure_ascii=False,
    ...
    def do_POST(self) -> None:
        try:
            ...
            if random_sample_match is not None:
                ...
                return
            is_chart_binding = (
                self.server.context["config"]["schema_version"] == "1.10.0"
            )
...
def serve(
    ...
    if enable_chart_only_runtime and schema_version != "1.10.0":
        raise Phase5DashboardError(
            "chart-only production binding에는 dashboard v1.10.0 계약이 필요합니다."
        )
