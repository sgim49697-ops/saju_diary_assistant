// dashboard.js - v1.17 후보에서 의도 확인과 날짜 재연결 안내를 구분한다.
// dashboard.js - K0·LoRA r16 원출력과 MIX2K v4 데이터를 진단한다.
...
let selectedPromptProfile = "guided_runtime_v2";
...
function updateRuntimeConversationBinding(message = null) {
  ...
  byId("manual-binding-copy").textContent = message || (
    connected
      ? `계산한 승인 원국과 ${connectedRuntimeDate} 일진이 이 새 대화에 고정됐습니다. 모델 원출력은 그대로 저장하고 grounding 경고를 함께 표시합니다.`
...
function renderRuntimeStatus(status) {
  ...
  byId("diagnostic-engine-note").textContent = status.enabled
    ? "원국·날짜 연결 뒤 K0와 LoRA r16 비교를 선택하면 두 모델에 동일한 canonical 사실 snapshot을 전달합니다. 원출력은 수정하지 않으며 운영 승격 근거가 아닙니다."
...
function renderDiagnostics(diagnostics) {
  ...
  return `<small class="message-diagnostics">${number(diagnostics.input_tokens, 0)} input tokens · ${number(diagnostics.elapsed_seconds, 1)}초 · peak ${bytes(diagnostics.peak_allocated_bytes)}${gpu}${audit}</small>${warning}`;
}
...
function renderEngineSelection(selectionId, locked) {
  ...
  byId("manual-prompt-label").textContent = metadata?.mode === "paired" ? "선택한 두 모델에 함께 보낼 질문" : `${metadata?.label || "선택 모델"}에 보낼 질문`;
...
byId("new-session-button").addEventListener("click", () => {
  ...
  selectedPromptProfile = promptProfileCatalog?.default_profile || "guided_runtime_v2";
...
byId("runtime-connect-button").addEventListener("click", async () => {
  ...
  try {
    ...
    byId("session-select").value = "";
    byId("manual-prompt").value = "";
    selectedPromptProfile = promptProfileCatalog?.bound_profile || "bound_chart_v2";
    selectedEngineSelection = inferenceEngineCatalog?.default_selection || "ki20_final";
...
byId("manual-form").addEventListener("submit", async (event) => {
  ...
  byId("session-meta").textContent = selection?.mode === "paired" ? "선택한 두 모델을 순서대로 불러오고 있습니다." : "선택한 모델을 별도 프로세스에서 불러오고 있습니다.";
  try {
