// dashboard.js - v1.18 후보의 직접 사실 응답·R16 생성·새 연결을 구분한다.
...
function updateRuntimeConversationBinding(message = null) {
  ...
  connectButton.disabled = !runtimeExactChartReady || !activeRuntimeSessionId || connected;
  byId("runtime-chart-connect-button").disabled = !runtimeChartReady || !activeRuntimeSessionId || connected;
  ...
  } else if (generateButton.textContent !== "응답 준비 중…") {
...
byId("runtime-connect-button").addEventListener("click", async () => {
  if (!runtimeExactChartReady || !activeRuntimeSessionId) return;
...
byId("manual-form").addEventListener("submit", async (event) => {
  ...
  try {
    const result = await api("/api/generate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ prompt, session_id: activeSessionId, profile: selectedPromptProfile, engine_selection: selectedEngineSelection, runtime_session_id: connectedRuntimeSessionId, runtime_binding_scope: connectedRuntimeScope }),
    });
