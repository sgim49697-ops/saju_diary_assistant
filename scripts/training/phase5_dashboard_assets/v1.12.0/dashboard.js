...
function updateRuntimeConversationBinding(message = null) {
  ...
  pill.textContent = connected ? `원국·${connectedRuntimeScope} 연결됨` : "원국·기간 대화 미연결";
...
function renderRuntimeStatus(status) {
  ...
  if (!status.enabled) {
    ...
    disconnectRuntimeConversation("원국·일별 기간 runtime이 비활성화되어 대화 연결을 해제했습니다.");
  }
  byId("diagnostic-engine-note").textContent = status.enabled
    ? "원국·기간 연결 뒤 K0와 KI20 비교를 선택하면 두 모델에 동일한 canonical 사실 snapshot을 전달합니다. 모델 해석은 품질 승격 근거가 아닙니다."
...
byId("runtime-connect-button").addEventListener("click", async () => {
  ...
  try {
    ...
    renderConversation(null);
    updateRuntimeConversationBinding();
    byId("runtime-status-copy").textContent = `${selectedScope} 일별 label을 승인 원국과 새 대화에 연결했습니다.`;
