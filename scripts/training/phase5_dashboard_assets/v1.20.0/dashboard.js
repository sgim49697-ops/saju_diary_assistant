// dashboard.js - v1.20 후보의 직접 사실 응답·R16 생성·새 연결을 구분한다.
...
let selectedPromptProfile = "product_v1";
let selectedEngineSelection = "lora_r16";
let promptProfileCatalog = null;
let inferenceEngineCatalog = null;
let datasetCatalog = null;
let selectedDatasetSplit = "mix2k_v4_train";
...
let runtimeExactChartReady = false;
...
let connectedRuntimeScope = "chart";
...
// 화면 소유권만 무효화한다. 서버의 정상 저장·계산을 취소하거나 재요청하지 않는다.
function invalidateView() {
  ...
  byId("generate-button").textContent = "답변 생성";
  byId("runtime-chart-button").disabled = !runtimeCanaryStatus?.enabled;
  byId("runtime-chart-button").textContent = activeRuntimeSessionId ? "원국 다시 계산" : "원국 계산";
  byId("runtime-delete-button").disabled = !runtimeCanaryStatus?.enabled;
  byId("runtime-delete-button").textContent = "출생 세션 즉시 삭제";
}

function requestOwner() {
  return { epoch: viewEpoch, session: activeSessionId, runtime: connectedRuntimeSessionId, scope: connectedRuntimeScope, date: connectedRuntimeDate };
}

function ownsView(owner) {
  return owner.epoch === viewEpoch && owner.session === activeSessionId
    && owner.runtime === connectedRuntimeSessionId && owner.scope === connectedRuntimeScope && owner.date === connectedRuntimeDate;
}
...
function updateRuntimeConversationBinding(message = null) {
  ...
  pill.textContent = connected ? (connectedRuntimeScope === "chart_day" ? `원국·${connectedRuntimeDate} 연결됨` : "원국 연결됨 · 날짜 미연결") : "원국 대화 미연결";
  pill.className = `status-pill ${connected ? "connected" : "locked"}`;
  const connectButton = byId("runtime-connect-button");
  connectButton.disabled = Boolean(runtimeOperation) || !runtimeExactChartReady || !activeRuntimeSessionId || connected;
  byId("runtime-chart-connect-button").disabled = Boolean(runtimeOperation) || !runtimeChartReady || !activeRuntimeSessionId || connected;
  ...
  byId("manual-binding-copy").textContent = message || (
    connected
      ? `승인 원국${connectedRuntimeScope === "chart_day" ? `과 ${connectedRuntimeDate} 일진` : ""}이 연결됐습니다. 일반 대화에서는 필요한 대화만 전달하며 연결은 유지합니다. 정정·날짜 변경은 새 대화로 연결합니다.`
...
function renderRuntimeStatus(status) {
  ...
  if (runtimeOperation) {
    byId("runtime-chart-button").disabled = true;
    byId("runtime-delete-button").disabled = true;
  }
  ...
  byId("diagnostic-engine-note").textContent = status.enabled
    ? "R16 단독 제품 후보입니다. 직접 조회와 모델 생성을 구분하고 현재 질문에 필요한 정보만 전달합니다. 운영 승격 근거가 아닙니다."
    : "K0·LoRA r16·KI20은 계산 엔진이 연결되지 않으면 진단용 언어 모델일 뿐입니다. 출력은 명리학적 정답이나 운영 승격 근거가 아닙니다.";
}

async function refreshRuntimeStatus() {
  const epoch = viewEpoch, sequence = ++runtimeStatusSequence;
  try {
    const status = await api("/api/runtime/status");
    if (epoch !== viewEpoch || sequence !== runtimeStatusSequence) return;
...
function renderRuntimeFacts(payload) {
  ...
  runtimeExactChartReady = chart?.status === "ok" && chart?.fact_authority === "HARD_GT";
  runtimeChartReady = runtimeExactChartReady || (chart?.status === "partial" && chart?.fact_authority === "POLICY_BOUND_RULE");
...
function renderModelChecks(payload) {
  ...
  byId("manual-panel").classList.remove("hidden");
  byId("prompt-example-panel").classList.toggle("hidden", !unlocked);
  byId("run-probe-button").classList.add("hidden");
...
function renderDatasetCatalog(payload) {
  ...
  if (payload.default_split && payload.splits.some((split) => split.split_id === payload.default_split)) selectedDatasetSplit = payload.default_split;
...
function renderDiagnostics(diagnostics) {
  ...
  const notices = (diagnostics.notices || []).map((notice) => `<div class="message-warning" role="note">${escapeHtml(notice)}</div>`).join("");
  if (diagnostics.response_kind && diagnostics.response_kind !== "model_generated") {
    return `<small class="message-diagnostics">모델 호출 없음 · ${escapeHtml(diagnostics.response_mode)}</small>${notices}`;
  }
  const gpu = Number.isFinite(diagnostics.gpu_total_memory_used_mib) ? ` · GPU ${number(diagnostics.gpu_total_memory_used_mib, 0)} MiB` : "";
  const warnings = Array.isArray(diagnostics.grounding_warnings) ? diagnostics.grounding_warnings : [];
  const warning = warnings.length ? `<div class="message-warning" role="note">RUNTIME_GROUNDING_WARNING · 원출력 보존 · ${escapeHtml(warnings.join(", "))}</div>` : "";
  const audit = diagnostics.grounding_passed === true ? " · grounding audit PASS" : "";
  return `<small class="message-diagnostics">${number(diagnostics.input_tokens, 0)} input tokens · ${number(diagnostics.elapsed_seconds, 1)}초 · peak ${bytes(diagnostics.peak_allocated_bytes)}${gpu}${audit}</small>${warning}${notices}`;
}
...
function renderConversation(session, contexts = null) {
  ...
  target.innerHTML = sessionTurns(session).map((turn) => {
    const assistants = turn.assistants.map((message) => {
      if (message.response_kind && message.response_kind !== "model_generated") {
        const label = { direct_fact: "계산 결과 직접 확인", clarification: "확인 안내", blocked: "지원 범위 안내" }[message.response_kind];
        return `<article class="message assistant program-response"><div class="message-head"><strong>${escapeHtml(label)}</strong><span>${escapeHtml(localTime(message.created_at_utc))}</span></div><p>${escapeHtml(message.content)}</p>${renderDiagnostics(message.diagnostics)}</article>`;
      }
      const engineId = message.engine_id || "lora_r16";
...
function renderEngineSelection(selectionId, locked) {
  ...
  const selections = (inferenceEngineCatalog?.selections || []).filter((item) => item.selection_id === "lora_r16");
  ...
  byId("manual-prompt-label").textContent = "질문 · 단순 사실은 직접 확인, 설명은 R16 생성";
...
async function renderSessions(payload, epoch = viewEpoch) {
  if (epoch !== viewEpoch || pendingGeneration) return;
  ...
  if (summary && summary.updated_at_utc !== loadedSessionUpdatedAt) {
    ...
    const session = await api(`/api/sessions/${owner.session}`);
    if (!ownsView(owner) || sequence !== sessionReadSequence || pendingGeneration) return;
...
async function refresh() {
  const epoch = viewEpoch, sequence = ++refreshSequence;
  ...
  try {
    ...
    if (epoch !== viewEpoch || sequence !== refreshSequence) return;
    renderStatus(status);
    renderMetrics(metrics);
    renderCheckpoints(checkpoints);
    renderModelChecks(checks);
    await renderSessions(sessions, epoch);
    if (epoch !== viewEpoch || sequence !== refreshSequence) return;
...
byId("new-session-button").addEventListener("click", () => {
  ...
  selectedPromptProfile = "product_v1";
  selectedEngineSelection = "lora_r16";
...
byId("session-select").addEventListener("change", async (event) => {
  ...
  const owner = requestOwner(), sequence = ++sessionReadSequence;
  try {
    const session = await api(`/api/sessions/${owner.session}`);
    if (!ownsView(owner) || sequence !== sessionReadSequence) return;
    loadedSessionUpdatedAt = session.updated_at_utc;
...
function startRuntimeConversation(scope, selectedDate = null) {
  ...
  renderConversation(null);
  updateRuntimeConversationBinding();
  byId("runtime-status-copy").textContent = "새 연결 대화를 시작했습니다. 이전 대화와 계산 snapshot은 보존되며 이력을 자동 이관하지 않습니다.";
  byId("manual-prompt").placeholder = scope === "chart_day" ? `예: ${selectedDate}의 일진을 설명해줘` : "예: 내 일간은 뭐야?";
  byId("manual-panel").classList.remove("hidden");
  byId("manual-prompt").focus();
}

byId("runtime-chart-connect-button").addEventListener("click", () => startRuntimeConversation("chart"));

byId("runtime-connect-button").addEventListener("click", async () => {
  if (!runtimeExactChartReady || !activeRuntimeSessionId || runtimeOperation) return;
  ...
  invalidateView();
  disconnectRuntimeConversation();
  const operation = { epoch: viewEpoch, runtime: activeRuntimeSessionId };
  runtimeOperation = operation;
  button.disabled = true;
  button.textContent = "날짜 계산·연결 중…";
  try {
    const result = await api(`/api/runtime/sessions/${operation.runtime}/events`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    if (runtimeOperation !== operation || operation.epoch !== viewEpoch) return;
    renderRuntimeFacts(result);
    if (!runtimeDayReady) throw new Error(result.decision?.message || "단일 일진 계산이 차단됐습니다.");
    startRuntimeConversation("chart_day", selectedDate);
    ...
    if (runtimeOperation === operation && operation.epoch === viewEpoch) {
      runtimeOperation = null;
      updateRuntimeConversationBinding();
    }
  }
...
byId("runtime-chart-form").addEventListener("submit", async (event) => {
  ...
  if (!runtimeCanaryStatus?.enabled || runtimeOperation) return;
  invalidateView();
  ...
  const operation = { epoch: viewEpoch };
  runtimeOperation = operation;
  button.disabled = true;
  button.textContent = "원국 계산 중…";
  ...
  try {
    ...
    if (runtimeOperation !== operation || operation.epoch !== viewEpoch) return;
    const pendingRuntimeSessionId = created.session_id;
    let pendingRevision = created.state_revision;
    let result = null;
    for (const runtimeEvent of events) {
      result = await api(`/api/runtime/sessions/${pendingRuntimeSessionId}/events`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ expected_revision: pendingRevision, event: runtimeEvent }),
      });
      if (runtimeOperation !== operation || operation.epoch !== viewEpoch) return;
      pendingRevision = result.state_revision;
    }
    const nextChart = result?.result?.chart || result?.result;
    if (!["ok", "partial"].includes(nextChart?.status)) throw new Error(result?.decision?.message || "새 원국을 계산하지 못했습니다. 이전 계산은 보존됩니다.");
    activeRuntimeSessionId = pendingRuntimeSessionId;
    activeRuntimeRevision = pendingRevision;
    renderRuntimeFacts(result);
    byId("runtime-status-copy").textContent = result.status === "ready"
      ? "원국만 새 대화에 연결할 수 있습니다. exact 원국은 날짜도 계산해 연결할 수 있으며, 이전 대화는 보존됩니다."
...
byId("runtime-delete-button").addEventListener("click", async () => {
  if (!activeRuntimeSessionId || runtimeOperation) return;
  invalidateView();
  disconnectRuntimeConversation();
  const operation = { epoch: viewEpoch, runtime: activeRuntimeSessionId };
  runtimeOperation = operation;
  ...
  try {
    await api(`/api/runtime/sessions/${operation.runtime}`, { method: "DELETE" });
    if (runtimeOperation !== operation || operation.epoch !== viewEpoch) return;
...
byId("manual-form").addEventListener("submit", async (event) => {
  ...
  byId("session-meta").textContent = "질문을 확인하고 있습니다. 직접 사실 조회는 모델을 호출하지 않습니다.";
  try {
    const result = await api("/api/generate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ prompt, session_id: owner.session, profile: selectedPromptProfile, engine_selection: selectedEngineSelection, runtime_session_id: owner.runtime, runtime_binding_scope: owner.scope }),
    });
    if (pendingGeneration !== owner || !ownsView(owner)) return;
    invalidateView();
    ...
    await renderSessions(await api("/api/sessions"), epoch);
  } catch (error) {
    if (pendingGeneration !== owner || !ownsView(owner)) return;
    const dateBlocked = ["RUNTIME_DATE_REBIND_REQUIRED", "RUNTIME_PERIOD_SCOPE_UNSUPPORTED", "RUNTIME_DATE_SELECTION_REQUIRED"].includes(error.code);
    const intentConfirmation = error.code === "RUNTIME_INTENT_CONFIRMATION_REQUIRED";
    const prefix = intentConfirmation ? "질문 의도 확인 · 모델 생성 안 함" : dateBlocked ? "날짜 연결 확인 · 모델 생성 안 함" : error.code === "GPU_BUSY_TRAINING" ? "학습 GPU 사용 중" : "생성 실패";
    byId("session-meta").textContent = `${prefix}: ${error.message}`;
    if (dateBlocked) updateRuntimeConversationBinding(error.message);
  } finally {
    if (pendingGeneration === owner && owner.epoch === viewEpoch) {
      pendingGeneration = null;
