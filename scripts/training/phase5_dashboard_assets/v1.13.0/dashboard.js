...
let runtimePeriodReady = false;
let runtimeRelationReady = false;
let connectedRuntimeSessionId = null;
let connectedManualSessionId = null;
let connectedRuntimeScope = null;
...
function updateRuntimeConversationBinding(message = null) {
  ...
  pill.textContent = connected ? `원국·${connectedRuntimeScope} 연결됨` : "원국·기간·관계 대화 미연결";
  ...
  connectButton.textContent = connected
    ? "이 원국·기간으로 대화 중"
    : runtimeRelationReady
      ? "이 원국·날짜 관계로 새 대화 시작"
      : "이 원국·기간으로 새 대화 시작";
  const unavailableBoundSession = selectedSessionRuntimeBound && !connected;
  byId("manual-binding-copy").textContent = message || (
    connected
      ? `계산한 승인 원국과 ${connectedRuntimeScope} 기간${runtimeRelationReady ? "·단일 날짜 관계" : ""}가 이 새 대화에 고정됐습니다. 모델 응답은 자동 grounding 검사를 통과해야 저장됩니다.`
      : unavailableBoundSession
        ? "이 원국·기간 대화의 암호화 capability가 현재 페이지에 없습니다. 원국을 다시 계산하고 새 연결 대화를 시작하세요."
        : "일반 대화입니다. 계산한 원국·기간·단일 날짜 관계는 위 연결 버튼을 눌러 새 대화에만 결합합니다."
  );
...
function renderRuntimeStatus(status) {
  ...
  [byId("runtime-start-date"), byId("runtime-end-date")].forEach((input) => {
    if (status.period_minimum) input.min = status.period_minimum;
    if (status.period_maximum) input.max = status.period_maximum;
    if (status.period_today_kst && !input.value) input.value = status.period_today_kst;
    input.disabled = !status.enabled || byId("runtime-date-expression").value !== "explicit";
  });
  byId("runtime-date-expression").disabled = !status.enabled;
  updateRuntimePeriodFields();
  byId("runtime-day-note").textContent = status.period_minimum && status.period_maximum
    ? `서버 KST 기준 ${status.period_minimum}~${status.period_maximum}, 최대 ${status.period_maximum_days || 31}일의 공식 날짜 label만 연결합니다.`
    : "서버 KST 기준 오늘부터 2049-12-31까지, 최대 31일의 공식 날짜 label만 연결합니다.";
  if (!status.enabled) {
    runtimeChartReady = false;
    runtimePeriodReady = false;
    runtimeRelationReady = false;
    disconnectRuntimeConversation("원국·기간·relation runtime이 비활성화되어 대화 연결을 해제했습니다.");
  }
  byId("diagnostic-engine-note").textContent = status.enabled
    ? "원국·기간 연결 뒤 K0와 KI20 비교를 선택하면 두 모델에 동일한 canonical 사실 snapshot을 전달합니다. 단일 날짜 관계는 존재 사실만 전달하며 모델 해석은 품질 승격 근거가 아닙니다."
...
function updateRuntimePeriodFields() {
  const explicit = byId("runtime-date-expression").value === "explicit";
  byId("runtime-explicit-range").classList.toggle("hidden", !explicit);
  [byId("runtime-start-date"), byId("runtime-end-date")].forEach((input) => {
    input.disabled = !runtimeCanaryStatus?.enabled || !explicit;
    input.required = explicit;
  });
}
...
function renderRuntimeFacts(payload) {
  ...
  const relation = payload.result.relation || null;
  runtimeChartReady = chart?.status === "ok" && chart?.fact_authority === "HARD_GT";
  runtimePeriodReady = period?.status === "ok" && period?.fact_authority === "HARD_GT" && Array.isArray(period?.days);
  runtimeRelationReady = relation?.status === "ok" && relation?.fact_authority === "PROFILE_DETERMINISTIC" && relation?.interpretation_not_included === true;
  ...
  byId("runtime-period-table-wrap").classList.toggle("hidden", !days.length);
  byId("runtime-period-table").innerHTML = days.map((day) => `<tr><td>${escapeHtml(day.date)}</td><td>${escapeHtml(day.year_ganzhi)}</td><td>${escapeHtml(day.month_ganzhi)}</td><td>${escapeHtml(day.day_ganzhi)}</td><td>${escapeHtml(day.authority)}</td></tr>`).join("");
  const relationCard = byId("runtime-relation-card");
  relationCard.classList.toggle("hidden", !runtimeRelationReady);
  byId("runtime-relation-range-note").classList.toggle("hidden", !runtimePeriodReady || days.length <= 1);
  if (runtimeRelationReady) {
    byId("runtime-relation-date").textContent = relation.selected_date;
    byId("runtime-relation-ten-gods").innerHTML = ["year", "month", "day"].map((part) => {
      const item = relation.period_ten_gods?.[part] || {};
      const label = ({ year: "연", month: "월", day: "일" })[part];
      return `<div><strong>${escapeHtml(label)}주 ${escapeHtml(item.ganzhi)}</strong><span>천간 ${escapeHtml(item.stem_ten_god)} · 지지 본기 ${escapeHtml(item.branch_main_hidden_stem)} ${escapeHtml(item.branch_ten_god)}</span></div>`;
    }).join("");
    const direct = Array.isArray(relation.direct_relations) ? relation.direct_relations : [];
    byId("runtime-direct-relations").innerHTML = direct.length
      ? direct.map((item) => `<li><strong>${escapeHtml(item.natal_pillar)}지 ${escapeHtml(item.natal_branch)}</strong><span>↔ 일지 ${escapeHtml(item.period_branch)} · ${escapeHtml(item.relation)}</span></li>`).join("")
      : "<li><span>고정 관계표에서 일지 직접 관계가 없습니다.</span></li>";
    const repetitions = Array.isArray(relation.repetitions) ? relation.repetitions : [];
    byId("runtime-repetitions").innerHTML = repetitions.length
      ? repetitions.map((item) => `<li><strong>${escapeHtml(item.period_part)} ${escapeHtml(item.component)}</strong><span>↔ 원국 ${escapeHtml(item.natal_pillar)} · ${escapeHtml(item.value)}</span></li>`).join("")
      : "<li><span>동일 천간·지지 반복이 없습니다.</span></li>";
  } else {
    byId("runtime-relation-date").textContent = "";
    byId("runtime-relation-ten-gods").replaceChildren();
    byId("runtime-direct-relations").replaceChildren();
    byId("runtime-repetitions").replaceChildren();
  }
...
byId("runtime-date-expression").addEventListener("change", () => {
  updateRuntimePeriodFields();
  runtimePeriodReady = false;
  runtimeRelationReady = false;
  disconnectRuntimeConversation("선택 기간이 바뀌었습니다. 같은 원국으로 새 연결 대화를 시작하세요.");
});
[byId("runtime-start-date"), byId("runtime-end-date")].forEach((input) => input.addEventListener("change", () => {
  runtimePeriodReady = false;
  runtimeRelationReady = false;
  disconnectRuntimeConversation("직접 날짜 범위가 바뀌었습니다. 같은 원국으로 새 연결 대화를 시작하세요.");
}));

function selectedPeriodRequest() {
  const dateExpression = byId("runtime-date-expression").value;
  const request = { schema_version: "saju-period-request-v2", date_expression: dateExpression, start_date: null, end_date: null };
  if (dateExpression === "explicit") {
    const start = byId("runtime-start-date");
    const end = byId("runtime-end-date");
    if (!start.reportValidity() || !end.reportValidity()) return null;
    request.start_date = start.value;
    request.end_date = end.value;
  }
  return request;
}

function periodScopeLabel(period) {
  const scope = period?.period_scope;
  if (!scope) return "선택 기간";
  return scope.start_date === scope.end_date ? scope.start_date : `${scope.start_date}~${scope.end_date}`;
}
...
byId("runtime-connect-button").addEventListener("click", async () => {
  ...
  const request = selectedPeriodRequest();
  if (!request) return;
  button.disabled = true;
  button.textContent = "기간 계산·연결 중…";
  try {
    ...
    if (!runtimePeriodReady) throw new Error(result.decision?.message || "일별 기간 계산이 차단됐습니다.");
    const selectedScope = periodScopeLabel(result.result.period);
    activeSessionId = null;
    loadedSessionUpdatedAt = null;
    startingNewSession = true;
    ...
    connectedManualSessionId = null;
    connectedRuntimeScope = selectedScope;
    byId("session-select").value = "";
    byId("manual-prompt").value = "";
    ...
    renderConversation(null);
    updateRuntimeConversationBinding();
    byId("runtime-status-copy").textContent = runtimeRelationReady
      ? `${selectedScope} 일별 label과 원국 관계 사실을 새 대화에 연결했습니다.`
      : `${selectedScope} 일별 label을 승인 원국과 새 대화에 연결했습니다. 범위 관계 배열은 생성하지 않았습니다.`;
    byId("manual-prompt").placeholder = `예: ${selectedScope}의 날짜별 흐름을 원국과 함께 설명해줘`;
    byId("manual-prompt").focus();
    byId("manual-panel").scrollIntoView({ behavior: "smooth", block: "start" });
  } catch (error) {
    runtimePeriodReady = false;
    runtimeRelationReady = false;
    disconnectRuntimeConversation(`기간 계산·연결 실패: ${error.message}`);
    byId("runtime-status-copy").textContent = `기간 계산·연결 실패: ${error.message}`;
  } finally {
    updateRuntimeConversationBinding();
  }
});
...
byId("runtime-chart-form").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    byId("runtime-status-copy").textContent = result.status === "ready"
      ? "승인 원국 사실을 계산했습니다. 기간을 고른 뒤 ‘이 원국·기간으로 새 대화 시작’을 누르세요. exact 출생시각 원국만 기간과 연결할 수 있습니다."
...
byId("runtime-delete-button").addEventListener("click", async () => {
  ...
  try {
    ...
    runtimePeriodReady = false;
    runtimeRelationReady = false;
    disconnectRuntimeConversation("출생 세션과 원국·기간 대화 연결을 즉시 삭제했습니다.");
    byId("runtime-facts-panel").classList.add("hidden");
