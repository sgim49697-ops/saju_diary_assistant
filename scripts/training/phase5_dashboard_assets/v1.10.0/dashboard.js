...
function updateRuntimeConversationBinding(message = null) {
  ...
  pill.textContent = connected ? "원국 연결됨" : "원국 대화 미연결";
  ...
  connectButton.textContent = connected ? "이 원국으로 대화 중" : "이 원국으로 새 대화 시작";
  const unavailableBoundSession = selectedSessionRuntimeBound && !connected;
  byId("manual-binding-copy").textContent = message || (
    connected
      ? "계산한 승인 원국이 이 새 대화에 고정됐습니다. 모델 응답은 자동 grounding 검사를 통과해야 저장됩니다."
      : unavailableBoundSession
        ? "이 원국 대화의 암호화 capability가 현재 페이지에 없습니다. 위에서 원국을 다시 계산하고 새 연결 대화를 시작하세요."
        : "일반 대화입니다. 계산한 원국은 위 연결 버튼을 눌러 새 대화에만 결합합니다."
  );
...
function renderRuntimeStatus(status) {
  ...
  if (!status.enabled) {
    ...
    disconnectRuntimeConversation("원국 runtime이 비활성화되어 대화 연결을 해제했습니다.");
  }
  byId("diagnostic-engine-note").textContent = status.enabled
    ? "원국 계산 뒤 K0와 KI20 비교를 선택하면 두 모델에 동일한 canonical 사실 snapshot을 전달합니다. 모델 해석은 품질 승격 근거가 아닙니다."
...
function renderRuntimeFacts(payload) {
  ...
  runtimeChartReady = payload.status === "ready";
...
byId("runtime-connect-button").addEventListener("click", () => {
  ...
  renderConversation(null);
  updateRuntimeConversationBinding();
  byId("manual-prompt").placeholder = "예: 내 오늘 사주 봐줄래?";
  byId("manual-prompt").focus();
...
byId("runtime-chart-form").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    byId("runtime-status-copy").textContent = result.status === "ready"
      ? "승인 원국 사실을 계산했습니다. ‘이 원국으로 새 대화 시작’을 눌러 모델에 명시적으로 연결하세요."
...
byId("runtime-delete-button").addEventListener("click", async () => {
  ...
  try {
    ...
    runtimeChartReady = false;
    disconnectRuntimeConversation("출생 세션과 원국 대화 연결을 즉시 삭제했습니다.");
    byId("runtime-facts-panel").classList.add("hidden");
