# dashboard_grounding_v4.py - 활성 요청 절·직전 사용자 맥락을 읽기 전용으로 판별하고 부모 사실 문법을 보존한다.
...
_DOMAIN = r"사주|운세|일진|원국|명식|운기|흐름|팔자|오행|일간|간지|천간|지지|해석"
_OPT_OUT = re.compile(
    rf"(?:{_DOMAIN})(?:\s*(?:랑|와|과|하고|·|/|,)?\s*(?:{_DOMAIN}))*"
    r"\s*(?:얘기|이야기|풀이|해석)?\s*(?:은|는|을|를)?\s*"
    r"(?:말고|빼고|제외하고|제외해|그만하고|그만해|"
    r"(?:잠깐\s*)?쉬고\s*싶|하지\s*말|보지\s*말|보고\s*싶지\s*않|안\s*볼래)"
)
_FORECAST = re.compile(r"운세|일진|운기")
_CHART = re.compile(r"일간|일주|원국|명식|팔자|오행|사주|간지|천간|지지")
_CLAUSE_BREAK = re.compile(r"(?<!\d)[.!?。！？;]+|[.!?](?!\d)|\n+")
_FOLLOWUP = re.compile(
    r"^(?:(?:그럼|그러면|그렇다면|혹시)\s*)?"
    r"(?:오늘|내일|모레|어제)(?:은|는|도|이|가)?\s*"
    r"(?:(?:좀|조금|더)\s*)?"
    r"(?:어때|어떨|어떻게|나아|좋아질|괜찮|봐\s*줘)[가-힣\s]*$"
)
INTENT_CONFIRMATION = "RUNTIME_INTENT_CONFIRMATION_REQUIRED"


@dataclass(frozen=True)
class RequestIntent:
    """계산이나 모델 입력을 바꾸지 않는 요청별 판별 결과."""
    intent: str
    period_text: str = ""
    context_source: str = "current_request"


def resolve_intent(prompt: str, *, prior_user_request: str | None = None) -> RequestIntent:
    """현재 활성 요청을 우선하고 짧은 날짜 후속에만 직전 사용자 주제를 참조한다."""
    if not isinstance(prompt, str) or not prompt.strip():
        raise ValueError("비어 있지 않은 질문 문자열이 필요합니다.")
    if prior_user_request is not None and not isinstance(prior_user_request, str):
        raise ValueError("이전 사용자 요청은 검증된 문자열이어야 합니다.")
    previous = None
    if prior_user_request and prior_user_request.strip():
        previous = resolve_intent(prior_user_request).intent
        if previous == "clarification_required":
            previous = None
    normalized = re.sub(r"[^\S\n]+", " ", prompt.casefold()).strip()
    active, exclusions = _OPT_OUT.subn(" ", normalized)
    topic = "general_followup" if exclusions else previous
    source = "current_request" if exclusions else "previous_user" if previous else "none"
    forecasts = []
    chart = False
    unresolved = False
    inherited = False
    for raw in _CLAUSE_BREAK.split(active):
        clause = raw.strip()
        if not clause:
            continue
        temporal = bool(_RELATIVE.search(clause) or _DATE.search(clause) or _RANGE.search(clause))
        if _FORECAST.search(clause) or (
            temporal and re.search(r"사주", clause)
            and not re.search(r"태어|출생|생년|생일|양력|음력", clause)
        ):
            forecasts.append(clause)
            topic, source = "period_request", "current_request"
        elif _CHART.search(clause):
            chart = True
            topic, source = "chart_interpretation", "current_request"
        elif _FOLLOWUP.fullmatch(clause):
            if topic in {"period_request", "chart_interpretation"}:
                forecasts.append(clause)
                inherited |= source == "previous_user"
            elif topic != "general_followup":
                unresolved = True
        else:
            # 날짜·흐름·해석이라는 단어만으로 사주 주제를 만들지 않는다.
            topic, source = "general_followup", "current_request"
    if forecasts:
        return RequestIntent(
            "period_request", "\n".join(forecasts),
            "previous_user" if inherited else "current_request",
        )
    if unresolved:
        return RequestIntent("clarification_required", context_source="none")
    return RequestIntent("chart_interpretation" if chart else "general_followup")


def prompt_intent(prompt: str, *, prior_user_request: str | None = None) -> str:
    return resolve_intent(prompt, prior_user_request=prior_user_request).intent
...
def date_scope(
    ...
    result: dict[str, Any] = {
        "policy_version": "day-request-scope-v1.2.0",
    ...
    decision = resolve_intent(prompt, prior_user_request=prior_user_request)
    if decision.intent == "clarification_required":
        result.update(
            allowed=False, reason_code=INTENT_CONFIRMATION,
            message="운세가 궁금한가요, 아니면 일상 이야기인가요? "
            "‘내일 운세’ 또는 ‘내일 면접’처럼 주제를 함께 알려주세요.",
        )
        return result
    if binding is None or decision.intent != "period_request":
        return result
    prompt = decision.period_text
    code = None
...
def audit_output(
    ...
    intent = prompt_intent(prompt, prior_user_request=prior_user_request)
