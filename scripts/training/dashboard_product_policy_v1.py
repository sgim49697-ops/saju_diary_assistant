# dashboard_product_policy_v1.py - 승인 사실 조회와 요청별 정보·이력 선택을 모델 없이 결정한다.
...
POLICY_VERSION = "saju-product-response-v1.0.0"
...
_EXPLAIN = re.compile(r"설명|해석|풀어|성향|특징|장점|단점|직업|조언|보완|예시|사례|차이|의미|관계|왜|어떻게|쉽게|문장|메시지|재작성|추천")
_REFER = re.compile(r"아까|방금|앞서|그\s*(?:말|설명|답변)|다시|더\s*쉽게|짧게|다듬|말투|그중|중에서")
_UNSUPPORTED = re.compile(r"신강약|격국|용신|대운|사건\s*예측|미래\s*사건")
...
def _labels(prompt: str) -> list[str]:
    found = [name for name in FIELD_PATHS if name in prompt]
    if "년주" in prompt and "연주" not in found:
        found.append("연주")
    return found
...
def render_fact(prompt: str, label: str, actual: str) -> str:
    ...
    return f"{correction}{prefix} {label}은 {rendered}입니다."
...
def plan_response(prompt: str, binding: dict | None, messages: list[dict], *, today=None) -> ResponsePlan:
    ...
    decision = intent.resolve_intent(prompt, prior_user_request=previous)
    opt_out = bool(intent._OPT_OUT.search(prompt))
    refer = bool(_REFER.search(prompt)) and not opt_out
    if not opt_out and _labels(prompt) and decision.intent == "general_followup":
        decision = intent.RequestIntent("chart_interpretation")
    if refer and decision.intent == "general_followup" and last.get("response_mode") in {"chart_explanation", "day_explanation", "fact_lookup"}:
        decision = intent.RequestIntent("chart_interpretation")
    # 명시적으로 승인된 용어를 가리키는 후속도 현재 문장의 '사주' 유무와 분리한다.
    if refer and any(term in prompt for term in _NAMES):
        decision = intent.RequestIntent("chart_interpretation")
    scope = intent.date_scope(prompt, binding, today=today, prior_user_request=previous)
    ...
    if selected_day_reference and not intent._RELATIVE.search(prompt) and not intent._DATE.search(prompt) and not intent._RANGE.search(prompt) and code == intent.DATE_AMBIGUOUS:
        code = None
    if _UNSUPPORTED.search(intent._OPT_OUT.sub(" ", prompt)):
        code = "RUNTIME_CALCULATION_UNSUPPORTED"
    if code:
        clauses = [c.strip() for c in intent._CLAUSE_BREAK.split(prompt) if c.strip()]
        general = [c for c in clauses if intent.resolve_intent(c).intent == "general_followup" and not _UNSUPPORTED.search(c) and re.search(r"써\s*줘|작성|다듬|들어\s*줘|위로|공감|요약", c)]
