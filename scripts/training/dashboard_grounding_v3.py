# dashboard_grounding_v3.py - 일반 대화의 의도 오탐을 줄이고 v2 날짜·사실 검사 규칙은 유지한다.
...
SCORER_VERSION = "saju-bound-chart-grounding-v3.0.0"
INTENT_POLICY_VERSION = "saju-request-intent-v1.0.0"
...
_ORDINARY = re.compile(
    r"메시지|문자|메일|회의|실수|위로|잠이|두\s*문장.*써|하소연|들어\s*줘|"
    r"힘들|지쳤|피곤|속상|짜증|이불킥|고마워|고맙|수고했|기분이|마음이|"
    r"말투|문장|다듬어|덜\s*딱딱|날씨|식사|점심|저녁\s*메뉴|그냥\s*(?:얘기|이야기)"
)
_AMBIGUOUS_FOLLOWUP = re.compile(
    r"(?<![가-힣])(?:오늘|내일|모레|어제)(?:은|는|도|이|가)?\s*"
    r"(?:(?:좀|조금|더)\s*)?(?:어때|어떨|어떻게|나아|좋아질|괜찮|봐\s*줘|[?？])"
)


def prompt_intent(prompt: str) -> str:
    """명시적으로 제외한 주제만 지운다. 별도의 운세 요청이나 애매한 날짜 후속은 차단 경로다."""
    ...
    # '사주 말고 내일 운세'·'그냥 내일 운세를 메시지로'도 날짜 검사를 거친다.
    if re.search(r"운세|일진|운기", active):
        return "period_request"
    if _AMBIGUOUS_FOLLOWUP.search(active):
        return "period_request"
    if re.search(r"일간|원국|명식|팔자|오행", active) and not re.search(
        r"흐름", active
    ):
        return "chart_interpretation"
    if not re.search(_DOMAIN, active):
        if _ORDINARY.search(active):
            return "general_followup"
        if exclusions and not (
            temporal
            and re.search(
                r"어때|어떨|어떻게|나아|좋아질|괜찮|봐\s*줘|알려|확인|보여|해\s*줘|해줄",
                active,
            )
        ):
            return "general_followup"
    if re.search(
        r"쉬운\s*말|쉽게|한자.*어려|풀어\s*(?:말|설명)", active
    ) and not re.search(r"내일|모레|이번\s*주|다음", active):
        return "general_followup"
    if temporal or re.search(r"기간|\d{1,2}\s*월\s*\d{1,2}\s*일", active):
        return "period_request"
    if re.search(r"사주|간지|천간|지지|성향|해석", active):
        return "chart_interpretation"
    return "general_followup"
...
def date_scope(
    ...
    result: dict[str, Any] = {
        "policy_version": "day-request-scope-v1.1.0",
