# phase5_dashboard_v1_16.py - v1.15 원출력 계약을 보존하고 질문 의도만 분리하는 격리 후보다.
...
V116_ASSET_ROOT = ASSET_ROOT / "v1.15.0"
...
def _validate_prompt_profiles(
    ...
    if schema_version == "1.16.0":
        if (
            ...
            raise Phase5DashboardError(
                "Dashboard v1.16 prompt profile 계약이 다릅니다."
            )
        expected = {
            "guided_runtime_v2": (
                "구조화 입력 진단 v2",
        ...
        for profile_id, details in expected.items():
            ...
            if (
                ...
                raise Phase5DashboardError(
                    f"Dashboard v1.16 prompt 세부 계약이 다릅니다: {profile_id}"
                )
        ...
        if (
            ...
            raise Phase5DashboardError("Dashboard v1.16 raw prompt 계약이 다릅니다.")
        return
    if (
...
def _validate_chart_only_runtime(
    ...
    is_day_runtime = schema_version in {"1.11.0", "1.16.0"}
    expected_binding = (
    ...
    expected_asset_root = (
        "scripts/training/phase5_dashboard_assets/v1.15.0"
        if schema_version == "1.16.0"
    ...
    if (
        ...
        or (
            schema_version in {"1.10.0", "1.11.0", "1.16.0"}
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise Phase5DashboardError("Dashboard v1.16 config 계약이 다릅니다.")
    ...
    if remote_share != expected_remote_share:
        raise Phase5DashboardError("Dashboard v1.16 원격 공유 계약이 다릅니다.")
    if any(
        ...
        raise Phase5DashboardError("Dashboard v1.16 진단 governance가 다릅니다.")
    ...
    if (
        ...
        != "dashboard/v1.16.0/grounding-v3.0.0/manual_sessions"
        ...
        raise Phase5DashboardError("Dashboard v1.16 세션 계약이 다릅니다.")
    ...
    if config.get("runtime_canary") is not None:
        raise Phase5DashboardError(
            "Dashboard v1.16에 legacy runtime canary가 있습니다."
        )
    ...
    if generation != {
        ...
        raise Phase5DashboardError("Dashboard v1.16 generation 계약이 다릅니다.")
...
def metrics_payload(context: dict[str, Any]) -> dict[str, Any]:
    if context["config"]["schema_version"] == "1.16.0":
        ...
        for offset in range(0, len(messages), chunk_size):
            ...
            for index, engine_id in enumerate(selection["engine_ids"], 1):
                ...
                if not isinstance(diagnostics, dict) or (
                    ...
                    raise Phase5DashboardError(
                        "v1.16 tokenizer·scorer·날짜 진단 identity가 다릅니다."
                    )
                if schema_version == "1.6.0":
...
def _check_date_scope(
    ...
    scope = date_scope(prompt, runtime_binding)
...
def execute_manual_generation(
    ...
    _check_date_scope(prompt, runtime_binding)
    try:
...
def _execute_manual_generation_with_gpu_lock_held(
    ...
    scope = _check_date_scope(prompt, runtime_binding)
    gate = _generation_gate(context)
    if not gate["allowed"]:
        raise Phase5DashboardError(
    ...
    if selected_runtime_binding is not None:
        if (
            context["config"]["schema_version"] not in {"1.10.0", "1.11.0", "1.16.0"}
            ...
            try:
                if path == "/api/generate":
                    ...
                    if is_chart_binding:
                        ...
                        _check_date_scope(payload["prompt"], runtime_binding)
...
def serve(
    ...
    if enable_chart_only_runtime:
        ...
        if schema_version in {"1.11.0", "1.16.0"}:
    ...
    asset_root = (
        ...
        if schema_version in {"1.10.0", "1.11.0", "1.16.0"}
        else ASSET_ROOT
    )
    server = DashboardHTTPServer(
        (host, port),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--artifact-root",
        type=Path,
        help="기존 모델·run 입력 root; 새 v1.16 세션 경로만 기록",
    )
