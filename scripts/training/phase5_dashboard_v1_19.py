# phase5_dashboard_v1_19.py - 동결된 부모와 분리해 R16 최소 제품 후보의 요청·입력·저장을 실행한다.
...
from scripts.training import dashboard_product_policy_v2 as product_policy
from scripts.training import dashboard_product_session_v2 as product_session
from scripts.training.dashboard_grounding_v4 import (
    SCORER_VERSION,
    audit_output,
    date_scope,
    prompt_intent,
...
DEFAULT_CONFIG = Path(
    ...
    "phase5-dashboard-v1.19.0-product-candidate.json"
)
ASSET_ROOT = Path(__file__).with_name("phase5_dashboard_assets")
V119_ASSET_ROOT = ASSET_ROOT / "v1.19.0"
...
GROUNDING_GATE_ID = SCORER_VERSION
...
def _validate_prompt_profiles(value, governance, schema_version) -> None:
    if (
        schema_version != "1.19.0" or value != product_policy.PROFILE_CONTRACT
        or governance.get("runtime_grounding_audit_required") is not True
        or governance.get("raw_model_output_preserved") is not True
    ):
        raise Phase5DashboardError("제품 후보 prompt 계약이 다릅니다.")
...
def _validate_chart_only_runtime(
    ...
    is_day_runtime = schema_version in {"1.11.0", "1.19.0"}
    expected_binding = (
    ...
    expected_engine = (
        "saju-runtime-python-v1.5.0" if is_day_runtime else "saju-runtime-python-v1.4.0"
    )
    expected_asset_root = (
        "scripts/training/phase5_dashboard_assets/v1.19.0"
        if schema_version == "1.19.0"
    ...
    if (
        ...
        or (
            schema_version in {"1.10.0", "1.11.0", "1.19.0"}
        ...
        or (is_day_runtime and governance.get("period_runtime_day_only") is not True)
...
def _validate_inference_engines(value: Any, governance: dict[str, Any]) -> None:
    ...
    if (
        value.get("default_selection") != "lora_r16"
...
def validate_config(config: dict[str, Any]) -> None:
    if config.get("product_policy") != {
        "policy_version": product_policy.POLICY_VERSION,
        "candidate_engine": "lora_r16",
        "session_schema": "1.9.0",
        "model_selection_allowed": False,
        "new_model_calls_authorized": False,
    }:
        raise Phase5DashboardError("제품 후보 구성 고정 계약이 다릅니다.")
    ...
    if (
        ...
        raise Phase5DashboardError("Dashboard v1.19 config 계약이 다릅니다.")
    ...
    if remote_share != expected_remote_share:
        raise Phase5DashboardError("Dashboard v1.19 원격 공유 계약이 다릅니다.")
    if any(
        ...
        raise Phase5DashboardError("Dashboard v1.19 진단 governance가 다릅니다.")
    ...
    if (
        ...
        != "dashboard/v1.19.0/product-v1.1.0/manual_sessions"
        ...
        raise Phase5DashboardError("Dashboard v1.19 세션 계약이 다릅니다.")
    ...
    if config.get("runtime_canary") is not None:
        raise Phase5DashboardError(
            "Dashboard v1.19에 legacy runtime canary가 있습니다."
        )
    ...
    if generation != {
        ...
        raise Phase5DashboardError("Dashboard v1.19 generation 계약이 다릅니다.")
...
def _prepare_chart_only_runtime(
    ...
    asset_root = _safe_under(
        root, contract["asset_root"], "dashboard chart-only assets"
    )
    if not asset_root.is_dir() or asset_root.is_symlink():
        raise Phase5DashboardError(
            "dashboard chart-only asset root가 안전하지 않습니다."
        )
    if not release_path.exists():
        return {
            "contract": contract,
...
def prepare_context(
    ...
    *,
    ...
    artifact_target = artifact_root or root
    if artifact_target.is_symlink() or not artifact_target.is_dir():
        raise Phase5DashboardError("artifact root가 없거나 symlink입니다.")
    artifact_target = artifact_target.resolve()
    config_target = _safe_under(root, config_path, "dashboard config")
    config = _load_json(config_target, "dashboard config")
    validate_config(config)
    run_target = _safe_under(artifact_target, run_root, "KI20 run")
    runs_root = (artifact_target / "runs").resolve(strict=False)
    try:
    ...
    inference_engines = _load_inference_engines(artifact_target, run_target, config)
...
def _direct_training_candidates(
    ...
    if (
        ...
        or manifest.get("production_promotion_allowed") is not False
        or manifest.get("sealed_blind_accessed") is not False
        or manifest.get("teacher_contract_mode") != "authorized_codex_only_fallback"
    ...
    for index, row in enumerate(rows, 1):
        ...
        if (
            ...
            raise Phase5DashboardError(f"MIX2K v4 학습 샘플 계약이 다릅니다: {index}")
        seen.add(record_id)
        candidates.append(
            {
                "identity": record_id,
...
def metrics_payload(context: dict[str, Any]) -> dict[str, Any]:
    if context["config"]["schema_version"] == "1.19.0":
        engine = _lora_r16_engine(context)
        trainer_state = (
            engine["resolved_path"].parent / "checkpoint-250/trainer_state.json"
        )
    ...
    if context["config"]["schema_version"] == "1.19.0":
        ...
        for path in candidates:
            ...
            items.append(
                {
                    "name": "final_adapter"
                    if path == engine["resolved_path"]
                    else path.name,
                    "status": "complete"
                    if not missing and not symlink_seen
                    else "incomplete",
                    ...
                    "preserved_milestone": path.name
                    in {"checkpoint-250", "final_adapter"},
                }
            )
        return {
            "items": items,
...
def status_payload(
    ...
    if context["config"]["schema_version"] == "1.19.0":
        ...
        _validate_lora_adapter_artifacts(context, engine)
        manifest = _load_json(
            engine["training_manifest_path"], "LoRA training manifest"
        )
...
def _generation_gate(context: dict[str, Any]) -> dict[str, Any]:
    ...
    marker = _load_json(marker_path, "보존 학습 start marker") if marker_path.exists() else {}
    service = _service_snapshot(marker.get("service_unit"))
    gpu = _gpu_snapshot()
    reasons: list[str] = []
    if service["active"]:
        reasons.append("training_service_active")
    if not _engine_availability(context, "lora_r16")["available"]:
        reasons.append("r16_candidate_not_available")
...
def _load_model(
    ...
    *,
    ...
    try:
        tokenizer = load_canonical_tokenizer(tokenizer_root or REPO_ROOT / K0_RELATIVE)
    except ValueError as exc:
...
def _load_engine_model(context: dict[str, Any], engine_id: str) -> tuple[Any, Any, Any]:
    engine = context["inference_engines"]["engines"][engine_id]
    if engine["kind"] != "lora_adapter":
        return _load_model(
            engine["resolved_path"],
            ...
            tokenizer_root=context["inference_engines"]["engines"]["k0_instruct"][
                "resolved_path"
            ],
        )
    _validate_lora_adapter_artifacts(context, engine)
    ...
    torch, tokenizer, model = _load_model(
        ...
        tokenizer_root=base["resolved_path"],
    )
    try:
...
def _generate_loaded(
    ...
    *,
    ...
    identity = input_identity(tokenizer, prepared, input_ids[0].tolist())
    ...
    return {
        ...
        "model_messages_sha256": product_policy.digest(prepared),
        "output_tokens": int(generated.shape[-1] - input_ids.shape[-1]),
        "stop_reason": "eos" if int(generated[0, -1]) in generation["eos_token_id"] else "max_tokens",
        **identity,
    }
...
def _validate_manual_session(
    ...
    if isinstance(value, dict) and value.get("schema_version") == "1.9.0":
        return product_session.validate_session(sys.modules[__name__], context, value, session_id)
    messages = value.get("messages")
    schema_version = {"1.7.0": "1.6.0", "1.8.0": "1.3.0"}.get(
        ...
        for offset in range(0, len(messages), chunk_size):
            ...
            for index, engine_id in enumerate(selection["engine_ids"], 1):
                ...
                if not isinstance(diagnostics, dict) or (
                    diagnostics.get("tokenizer_revision") != TOKENIZER_REVISION
                    or diagnostics.get("tokenizer_backend_sha256") != BACKEND_SHA256
                    or diagnostics.get("scorer_version") != SCORER_VERSION
                    or any(
                        re.fullmatch(r"[0-9a-f]{64}", str(diagnostics.get(key, "")))
                        is None
                        for key in ("rendered_prompt_sha256", "input_token_ids_sha256")
                    )
                    or not isinstance(diagnostics.get("date_scope"), dict)
                    or diagnostics["date_scope"].get("allowed") is not True
                ):
                    raise Phase5DashboardError(
                        "v1.19 tokenizer·scorer·날짜 진단 identity가 다릅니다."
                    )
                if schema_version == "1.6.0":
...
def _bound_prompt_intent(prompt: str) -> str:
    return prompt_intent(prompt)


def evaluate_bound_output(
    prompt: str, output: str, binding: dict[str, Any]
) -> dict[str, Any]:
    return audit_output(prompt, output, binding)


def audit_bound_output(
    prompt: str, output: str, binding: dict[str, Any], *,
    prior_user_request: str | None = None,
) -> dict[str, Any]:
    return audit_output(prompt, output, binding, prior_user_request=prior_user_request)


def _check_date_scope(
    prompt: str, runtime_binding: dict[str, Any] | None, *,
    prior_user_request: str | None = None,
) -> dict[str, Any]:
    if runtime_binding is not None:
        _runtime_model_context_from_binding(runtime_binding)
    scope = date_scope(prompt, runtime_binding, prior_user_request=prior_user_request)
    if not scope["allowed"]:
        raise DashboardRequestError(
            HTTPStatus.CONFLICT, scope["message"], reason_code=scope["reason_code"]
        )
    return scope


def _request_scope(context, prompt, session_id, binding, *, expected_state=None):
    plan, current, state = product_session.request_state(
        sys.modules[__name__], context, prompt, session_id, binding, expected=expected_state,
    )
    prior = next((m["content"] for m in reversed(current["messages"]) if m["role"] == "user"), None) if current else None
    return plan.trace(binding), prior, state
...
def execute_manual_generation(
    context, prompt, session_id=None, profile=None, engine_selection=None,
    runtime_session_id=None, runtime_binding=None, *,
    expected_request_state_sha256=None, defer_persistence=False,
):
    return product_session.execute(
        sys.modules[__name__], context, prompt, session_id, profile, engine_selection,
        runtime_session_id, runtime_binding,
        expected_request_state_sha256=expected_request_state_sha256,
...
def chart_only_runtime_status_payload(server: DashboardHTTPServer) -> dict[str, Any]:
    ...
    return {
        ...
        "code": code,
        "message": message,
        "release_id": (prepared["contract"]["release_id"] if configured else None),
        "facts_rendered_without_model": True,
...
class DashboardRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        except (OSError, ValueError, Phase5DashboardError, subprocess.SubprocessError) as exc:
    ...
    def do_POST(self) -> None:
        try:
            ...
            if path != "/api/generate":
                raise DashboardRequestError(
                    HTTPStatus.NOT_FOUND, "제품 후보에서 지원하지 않는 API입니다.",
                    reason_code="PRODUCT_ROUTE_UNSUPPORTED",
                )
            payload = self._request_json()
            result = product_session.http_generate(self, sys.modules[__name__], payload)
            self._send_json(HTTPStatus.OK, result)
...
def _manual_generation_subprocess(
    ...
    *, expected_request_state_sha256: str | None = None, defer_persistence: bool = False,
) -> dict[str, Any]:
    _, _, request_state_sha256 = _request_scope(
        context, prompt, session_id, runtime_binding,
        expected_state=expected_request_state_sha256,
    )
    command = [
        sys.executable,
        ...
        "--artifact-root",
        str(context.get("artifact_root", context["repo_root"])),
        "generate",
        "--execute",
    ]
    ...
    if runtime_binding is not None:
        if (
            context["config"]["schema_version"] not in {"1.10.0", "1.11.0", "1.19.0"}
        ...
        input=json.dumps(
            {
                ...
                "expected_request_state_sha256": request_state_sha256,
                "defer_persistence": defer_persistence,
            },
            ensure_ascii=False,
        ),
    ...
    if result.returncode != 0:
        try:
            conflict = json.loads(result.stderr.strip())
        except (ValueError, TypeError):
            conflict = None
        if isinstance(conflict, dict) and conflict.get("code") in {
            "RUNTIME_INTENT_CONFIRMATION_REQUIRED", "RUNTIME_REQUEST_STATE_CHANGED",
            "RUNTIME_SESSION_BINDING_MISMATCH", "RUNTIME_DATE_REBIND_REQUIRED",
            "RUNTIME_DATE_SELECTION_REQUIRED", "RUNTIME_PERIOD_SCOPE_UNSUPPORTED",
        }:
            raise DashboardRequestError(
                HTTPStatus.CONFLICT, str(conflict.get("error", "요청 상태 확인이 필요합니다.")),
                reason_code=conflict["code"],
            )
    ...
    if (
        not isinstance(value, dict)
        or value.get("persisted") is not (not defer_persistence)
...
def serve(
    ...
    if enable_chart_only_runtime:
        ...
        if schema_version in {"1.11.0", "1.19.0"}:
            from scripts.runtime.product_dashboard_binding_v1 import (
                ProductDashboardBinding as RuntimeDashboardBinding,
            )
        else:
    ...
    asset_root = (
        ...
        if schema_version in {"1.10.0", "1.11.0", "1.19.0"}
        else ASSET_ROOT
    )
    server = DashboardHTTPServer(
        (host, port),
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="R16 단독 v1.19 미배포 제품 후보 대시보드")
    parser.add_argument("--config", type=Path, default=DEFAULT_CONFIG)
    parser.add_argument("--run-root", type=Path)
    parser.add_argument(
        "--artifact-root",
        type=Path,
        help="기존 모델·run 입력 root; 새 v1.19 세션 경로만 기록",
    )
    ...
    serve_parser.add_argument("--port", type=int, default=8771)
    ...
    probe = subparsers.add_parser("probe", help="후보에서는 고정 20건 비교를 실행하지 않음")
    ...
    generate.add_argument("--enable-chart-only-runtime-binding", action="store_true")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        context = prepare_context(
            REPO_ROOT, config_path, run_root, artifact_root=args.artifact_root
        )
        if args.command == "status":
            ...
            raise Phase5DashboardError("제품 후보에는 고정 20건 재생성을 허용하지 않습니다.")
            ...
            if (
                ...
                or type(payload.get("defer_persistence", False)) is not bool
                or (
            ...
            result = execute_manual_generation(
                ...
                expected_request_state_sha256=payload.get("expected_request_state_sha256"),
                defer_persistence=payload.get("defer_persistence", False),
            )
        else:
            raise Phase5DashboardError("지원하지 않는 command입니다.")
    except DashboardRequestError as exc:
        print(json.dumps({"code": exc.reason_code, "error": str(exc)}, ensure_ascii=False), file=sys.stderr)
        return 1
    except (
        OSError,
        ValueError,
