# phase5_dashboard_v1_20.py - 동결된 부모와 분리해 R16 최소 제품 후보의 요청·입력·저장을 실행한다.
...
from scripts.training import dashboard_product_policy_v3 as product_policy
from scripts.training import dashboard_product_session_v3 as product_session
...
DEFAULT_CONFIG = Path(
    ...
    "phase5-dashboard-v1.20.0-product-candidate.json"
)
ASSET_ROOT = Path(__file__).with_name("phase5_dashboard_assets")
V120_ASSET_ROOT = ASSET_ROOT / "v1.20.0"
...
def _validate_prompt_profiles(value, governance, schema_version) -> None:
    if (
        schema_version != "1.20.0" or value != product_policy.PROFILE_CONTRACT
...
def _validate_chart_only_runtime(
    ...
    is_day_runtime = schema_version in {"1.11.0", "1.20.0"}
    expected_binding = (
    ...
    expected_asset_root = (
        "scripts/training/phase5_dashboard_assets/v1.20.0"
        if schema_version == "1.20.0"
    ...
    if (
        ...
        or (
            schema_version in {"1.10.0", "1.11.0", "1.20.0"}
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise Phase5DashboardError("Dashboard v1.20 config 계약이 다릅니다.")
    ...
    if remote_share != expected_remote_share:
        raise Phase5DashboardError("Dashboard v1.20 원격 공유 계약이 다릅니다.")
    if any(
        ...
        raise Phase5DashboardError("Dashboard v1.20 진단 governance가 다릅니다.")
    ...
    if (
        ...
        != "dashboard/v1.20.0/product-v1.2.0/manual_sessions"
        ...
        raise Phase5DashboardError("Dashboard v1.20 세션 계약이 다릅니다.")
    ...
    if config.get("runtime_canary") is not None:
        raise Phase5DashboardError(
            "Dashboard v1.20에 legacy runtime canary가 있습니다."
        )
    ...
    if generation != {
        ...
        raise Phase5DashboardError("Dashboard v1.20 generation 계약이 다릅니다.")
...
def metrics_payload(context: dict[str, Any]) -> dict[str, Any]:
    if context["config"]["schema_version"] == "1.20.0":
        ...
        for offset in range(0, len(messages), chunk_size):
            ...
            for index, engine_id in enumerate(selection["engine_ids"], 1):
                ...
                if not isinstance(diagnostics, dict) or (
                    ...
                    raise Phase5DashboardError(
                        "v1.20 tokenizer·scorer·날짜 진단 identity가 다릅니다."
                    )
                if schema_version == "1.6.0":
...
def _manual_generation_subprocess(
    ...
    *, expected_request_state_sha256: str | None = None, defer_persistence: bool = False,
    ...
    if runtime_binding is not None:
        if (
            context["config"]["schema_version"] not in {"1.10.0", "1.11.0", "1.20.0"}
...
def serve(
    ...
    if enable_chart_only_runtime:
        ...
        if schema_version in {"1.11.0", "1.20.0"}:
    ...
    asset_root = (
        ...
        if schema_version in {"1.10.0", "1.11.0", "1.20.0"}
        else ASSET_ROOT
    )
    server = DashboardHTTPServer(
        (host, port),
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="R16 단독 v1.20 미배포 제품 후보 대시보드")
    parser.add_argument("--config", type=Path, default=DEFAULT_CONFIG)
    parser.add_argument("--run-root", type=Path)
    parser.add_argument(
        "--artifact-root",
        type=Path,
        help="기존 모델·run 입력 root; 새 v1.20 세션 경로만 기록",
    )
