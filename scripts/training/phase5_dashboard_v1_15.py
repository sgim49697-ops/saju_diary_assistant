# phase5_dashboard_v1_15.py - K0·R16의 원국·단일 일진 raw 출력을 비교한다.
...
V115_ASSET_ROOT = ASSET_ROOT / "v1.15.0"
...
def _validate_prompt_profiles(
    ...
    if schema_version == "1.15.0":
        if (
            ...
            raise Phase5DashboardError(
                "Dashboard v1.15 prompt profile 계약이 다릅니다."
            )
        expected = {
            "guided_runtime_v2": (
                "구조화 입력 진단 v2",
        ...
        for profile_id, details in expected.items():
            ...
            if (
                ...
                raise Phase5DashboardError(
                    f"Dashboard v1.15 prompt 세부 계약이 다릅니다: {profile_id}"
                )
        ...
        if (
            ...
            raise Phase5DashboardError("Dashboard v1.15 raw prompt 계약이 다릅니다.")
        return
    if (
...
def _validate_chart_only_runtime(
    ...
    is_day_runtime = schema_version in {"1.11.0", "1.15.0"}
    expected_binding = (
    ...
    if (
        ...
        or (
            schema_version in {"1.10.0", "1.11.0", "1.15.0"}
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise Phase5DashboardError("Dashboard v1.15 config 계약이 다릅니다.")
    ...
    if remote_share != expected_remote_share:
        raise Phase5DashboardError("Dashboard v1.15 원격 공유 계약이 다릅니다.")
    if any(
        ...
        raise Phase5DashboardError("Dashboard v1.15 진단 governance가 다릅니다.")
    ...
    if (
        ...
        raise Phase5DashboardError("Dashboard v1.15 세션 계약이 다릅니다.")
    ...
    if config.get("runtime_canary") is not None:
        raise Phase5DashboardError(
            "Dashboard v1.15에 legacy runtime canary가 있습니다."
        )
    ...
    if generation != {
        ...
        raise Phase5DashboardError("Dashboard v1.15 generation 계약이 다릅니다.")
...
def metrics_payload(context: dict[str, Any]) -> dict[str, Any]:
    if context["config"]["schema_version"] == "1.15.0":
        ...
        for offset in range(0, len(messages), chunk_size):
            ...
            for index, engine_id in enumerate(selection["engine_ids"], 1):
                ...
                if not isinstance(diagnostics, dict) or (
                    ...
                    raise Phase5DashboardError(
                        "v1.15 tokenizer·scorer·날짜 진단 identity가 다릅니다."
                    )
                if schema_version == "1.6.0":
...
def _execute_manual_generation_with_gpu_lock_held(
    ...
    if selected_runtime_binding is not None:
        if (
            context["config"]["schema_version"] not in {"1.10.0", "1.11.0", "1.15.0"}
...
def serve(
    ...
    if enable_chart_only_runtime:
        ...
        if schema_version in {"1.11.0", "1.15.0"}:
    ...
    asset_root = (
        ...
        if schema_version in {"1.10.0", "1.11.0", "1.15.0"}
        else ASSET_ROOT
    )
    server = DashboardHTTPServer(
        (host, port),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--artifact-root",
        type=Path,
        help="기존 모델·run 입력 root; 새 v1.15 세션 경로만 기록",
    )
