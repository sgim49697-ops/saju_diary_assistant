# phase5_dashboard_v1_17.py - 원출력·binding을 보존하고 활성 요청과 직전 사용자 맥락의 오차단을 보완한다.
...
DEFAULT_CONFIG = Path(
    ...
    "phase5-dashboard-v1.17.0-intent-candidate.json"
)
ASSET_ROOT = Path(__file__).with_name("phase5_dashboard_assets")
V117_ASSET_ROOT = ASSET_ROOT / "v1.17.0"
...
def _validate_prompt_profiles(
    ...
    if schema_version == "1.17.0":
        if (
            ...
            or set(profiles) != {"guided_runtime_v2", "bound_chart_v2", "raw_no_system"}
            ...
            raise Phase5DashboardError(
                "Dashboard v1.17 prompt profile 계약이 다릅니다."
            )
        expected = {
            "guided_runtime_v2": (
                "구조화 입력 진단 v2",
        ...
        for profile_id, details in expected.items():
            ...
            if (
                ...
                raise Phase5DashboardError(
                    f"Dashboard v1.17 prompt 세부 계약이 다릅니다: {profile_id}"
                )
        ...
        if (
            ...
            raise Phase5DashboardError("Dashboard v1.17 raw prompt 계약이 다릅니다.")
        return
    if (
...
def _validate_chart_only_runtime(
    ...
    is_day_runtime = schema_version in {"1.11.0", "1.17.0"}
    expected_binding = (
    ...
    expected_asset_root = (
        "scripts/training/phase5_dashboard_assets/v1.17.0"
        if schema_version == "1.17.0"
    ...
    if (
        ...
        or (
            schema_version in {"1.10.0", "1.11.0", "1.17.0"}
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise Phase5DashboardError("Dashboard v1.17 config 계약이 다릅니다.")
    ...
    if remote_share != expected_remote_share:
        raise Phase5DashboardError("Dashboard v1.17 원격 공유 계약이 다릅니다.")
    if any(
        ...
        raise Phase5DashboardError("Dashboard v1.17 진단 governance가 다릅니다.")
    ...
    if (
        ...
        != "dashboard/v1.17.0/grounding-v4.0.0/manual_sessions"
        ...
        raise Phase5DashboardError("Dashboard v1.17 세션 계약이 다릅니다.")
    ...
    if config.get("runtime_canary") is not None:
        raise Phase5DashboardError(
            "Dashboard v1.17에 legacy runtime canary가 있습니다."
        )
    ...
    if generation != {
        ...
        raise Phase5DashboardError("Dashboard v1.17 generation 계약이 다릅니다.")
...
def metrics_payload(context: dict[str, Any]) -> dict[str, Any]:
    if context["config"]["schema_version"] == "1.17.0":
        ...
        for offset in range(0, len(messages), chunk_size):
            ...
            for index, engine_id in enumerate(selection["engine_ids"], 1):
                ...
                if not isinstance(diagnostics, dict) or (
                    ...
                    raise Phase5DashboardError(
                        "v1.17 tokenizer·scorer·날짜 진단 identity가 다릅니다."
                    )
                if schema_version == "1.6.0":
...
def _request_scope(
    ...
    binding: dict[str, Any] | None, *, expected_state: str | None = None,
) -> tuple[dict[str, Any], str | None, str]:
    """검증된 직전 사용자 요청만 읽고 HTTP/worker/저장 사이 변경을 거부한다."""
    if (
        not isinstance(prompt, str) or not prompt.strip()
        or len(prompt) > context.get("config", {}).get("server", {}).get("max_prompt_chars", 4000)
        or CONTROL_PATTERN.search(prompt)
    ):
        raise Phase5DashboardError("수동 질문 길이 또는 문자가 허용 범위를 벗어납니다.")
    if binding is not None:
        _runtime_model_context_from_binding(binding)
    current = manual_session_payload(context, session_id) if session_id is not None else None
    prior = None
    if current is not None:
        if current.get("runtime_binding_sha256") is not None and (
            binding is None
            or current["runtime_binding_sha256"] != binding["capability_sha256"]
            or current["runtime_snapshot_sha256"] != binding["snapshot_sha256"]
        ):
            raise DashboardRequestError(
                HTTPStatus.CONFLICT, "기존 대화의 원국·날짜 연결이 다릅니다.",
                reason_code="RUNTIME_SESSION_BINDING_MISMATCH",
            )
        users = [m["content"] for m in current["messages"] if m["role"] == "user"]
        prior = users[-1] if users else None
    state = hashlib.sha256(json.dumps(
        {"session": current, "binding": binding},
    ...
    if expected_state is not None and (
        ...
        raise DashboardRequestError(
            HTTPStatus.CONFLICT, "질문 확인 이후 대화 상태가 변경됐습니다.",
            reason_code="RUNTIME_REQUEST_STATE_CHANGED",
        )
    return _check_date_scope(prompt, binding, prior_user_request=prior), prior, state
...
def execute_manual_generation(
    ...
    *, expected_request_state_sha256: str | None = None,
) -> dict[str, Any]:
...
def _execute_manual_generation_with_gpu_lock_held(
    ...
    *, expected_request_state_sha256: str | None = None,
    ...
    scope, prior_user_request, request_state_sha256 = _request_scope(
        context, prompt, session_id, runtime_binding,
        expected_state=expected_request_state_sha256,
    )
    ...
    if selected_runtime_binding is not None:
        if (
            context["config"]["schema_version"] not in {"1.10.0", "1.11.0", "1.17.0"}
    ...
    _request_scope(
        context, prompt, original_session_id, runtime_binding,
        expected_state=request_state_sha256,
    )
    ...
    session = {
        "schema_version": "1.7.0" if runtime_binding_sha256 else "1.8.0",
    ...
    contexts = {
        engine_id: {
            **{
                key: generated_by_engine[engine_id][key]
                for key in (
                    "tokenizer_revision",
                    "tokenizer_backend_sha256",
                    "rendered_prompt_sha256",
            ...
            try:
                if path == "/api/generate":
                    ...
                    if is_chart_binding:
                        ...
                        _, _, request_state_sha256 = _request_scope(
                            self.server.context, payload["prompt"], payload["session_id"],
                            runtime_binding,
                        )
...
def serve(
    ...
    if enable_chart_only_runtime:
        ...
        if schema_version in {"1.11.0", "1.17.0"}:
    ...
    asset_root = (
        ...
        if schema_version in {"1.10.0", "1.11.0", "1.17.0"}
        else ASSET_ROOT
    )
    server = DashboardHTTPServer(
        (host, port),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--artifact-root",
        type=Path,
        help="기존 모델·run 입력 root; 새 v1.17 세션 경로만 기록",
    )
