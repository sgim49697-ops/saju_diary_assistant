# phase5_dashboard_v1_11.py - 승인 원국·단일 일진 명시 연결과 grounding Gate를 제공한다.
...
V111_ASSET_ROOT = ASSET_ROOT / "v1.11.0"
...
def _validate_chart_only_runtime(
    ...
    is_day_runtime = schema_version == "1.11.0"
    expected_binding = (
    ...
    expected_asset_root = (
        "scripts/training/phase5_dashboard_assets/v1.11.0"
        if is_day_runtime
        else (
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    elif schema_version in {"1.7.0", "1.8.0", "1.9.0", "1.10.0", "1.11.0"}:
    ...
    elif schema_version in {"1.9.0", "1.10.0", "1.11.0"}:
        if runtime_canary is not None:
            raise Phase5DashboardError(
                "dashboard chart-only config에 legacy runtime canary가 있습니다."
            )
        _validate_chart_only_runtime(chart_only_runtime, governance, schema_version)
...
def execute_manual_generation(
    ...
    if selected_runtime_binding is not None:
        if (
            context["config"]["schema_version"] not in {"1.10.0", "1.11.0"}
...
class DashboardRequestHandler(BaseHTTPRequestHandler):
    ...
    def _error(
        ...
        *,
        ...
        if self.server.context["config"]["schema_version"] in {"1.10.0", "1.11.0"}:
            value["code"] = code
        payload = json.dumps(
            value,
            ensure_ascii=False,
    ...
    def do_POST(self) -> None:
        try:
            ...
            if random_sample_match is not None:
                ...
                return
            is_chart_binding = (
                self.server.context["config"]["schema_version"] in {"1.10.0", "1.11.0"}
            )
    ...
    def do_DELETE(self) -> None:
        try:
            ...
            if self.server.context["config"]["schema_version"] not in {"1.10.0", "1.11.0"}:
                raise DashboardRequestError(
...
def serve(
    ...
    if enable_chart_only_runtime and schema_version not in {"1.10.0", "1.11.0"}:
        raise Phase5DashboardError(
