...
- 이 저장소의 기본 브랜치는 `master`다. 이후 작업은 원본 프로젝트 폴더의 `master`에서 직접 수정·검증·한국어 커밋·푸시하며, 브랜치 이름을 `main`으로 바꾸지 않는다.
- 사용자의 명시적 요청 또는 필수 저장소 정책에 대한 사전 확인 없이 새 브랜치·worktree를 만들지 않는다. 다른 세션·dirty 상태를 이유로 자동 분기하지 않는다.
- 기존 작업의 병합을 요청받으면 검증·PR 병합·원본 폴더 동기화·안전한 정리까지 완료 범위로 삼는다. 후보 코드의 병합은 운영 배포나 모델 승격이 아니다.
- 브랜치 정리 전에 병합/변경 동등성·복구 근거를 확인한다. 실행 서비스, 고유 데이터·모델·run, 미커밋 변경이 있는 worktree는 보존하고 예외를 보고한다.
...
4. 독립 작업은 겹치지 않는 파일에서 진행한다. 같은 working tree의 `HEAD`나 index 변경이 상대 세션을 방해할 수 있으면 상태를 알리고 대기한다. 임의 분기하지 않으며, 별도 브랜치·worktree가 꼭 필요하면 이유를 설명하고 먼저 확인받는다.
...
- push 전 `git status --short --branch`와 upstream 차이를 재확인한다. 원격이 앞섰으면 force push하지 말고, 작업 소유권과 깨끗한 상태를 확인해 안전하게 통합한 뒤 다시 검증한다. 이를 위해 새 브랜치·worktree를 자동 생성하지 않는다.
