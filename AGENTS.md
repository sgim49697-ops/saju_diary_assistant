...
## 핵심 구현 비공개와 인증된 코드 검토

- `sgim49697-ops/saju_diary_assistant`는 비공개 개발 저장소로 유지한다. 사용자 요청 없이 공개 전환하거나 공개 원격·미러·배포 산출물로 복제하지 않는다.
- 사주 계산·기간·관계 엔진, 계산 정책·표, 경계 테스트, 내부 설계·검증 상세는 이 비공개 저장소에서 함께 버전 관리한다. 보호 목적으로 핵심 코드를 추적 해제하거나 승인 산출물·hash chain을 바꾸지 않는다.
- ChatGPT의 GitHub 연결은 해당 저장소에 대한 인증된 접근 권한으로 사용한다. 연결 문제를 해결하기 위해 저장소를 다시 공개하지 않는다. 확인 절차는 [저장소 접근 운영](docs/repository_visibility.md)을 따른다.
- 문서·schema의 `public` 또는 “공개 보고서”는 해당 결과의 필드 범위나 원문 제외를 뜻하며, GitHub 전체 공개·외부 재배포 허가로 해석하지 않는다.
- 외부 소개 자료는 별도로 선별한다. 코드·내부 정책·상세 설계·테스트·검증 원문을 공개 자료에 자동 포함하지 않으며, 사용자가 지정한 공개 범위 안에서만 배포한다.
- push 전에 의도한 원격의 비공개 상태를 인증된 API로 확인한다. 현재 `origin`은 `gh api repos/sgim49697-ops/saju_diary_assistant --jq .private`가 `true`여야 한다. 원격이 바뀌면 대상부터 재확인하며, 조회 실패·공개 상태에서는 push를 보류하고 원인을 보고한다.
