...
- 이 저장소는 공개 상태를 유지한다. 일부 핵심 구현을 숨기거나 유출을 줄여 달라는 요청을 저장소 전체 비공개 전환 권한으로 해석하지 않는다. 공개 여부 변경은 사용자가 그 변경을 명시적으로 요청한 경우에만 수행한다.
- 핵심 구현 보호는 대상 파일·노출 경로를 정해 처리한다. 코드 분리 시 기존 앱 동작·외부 코드 검토·승인 산출물과 hash chain에 미치는 영향을 먼저 확인한다.
- 원본 데이터·비밀값 제외 규칙과 다른 세션 변경 보존 규칙은 그대로 따른다. 저장소가 공개라는 이유로 내부 자료의 무분별한 추가 업로드를 허용하지 않는다.
- 새 내부 계산 연구·상세 설계·실험 원문은 Git 제외 경로 `private/`에 보관하고 공개 코드·보고서에 자동 복제하지 않는다. 이 경로는 로컬 보관용이며 Git 백업 대상이 아니다. 기존 추적 코드와 정본 문서는 자동 이동하지 않으며, 이 제외 규칙이 기존 공개 코드를 숨기는 것은 아니다.
