...
Runtime 10~40 구현과 R8·R16·R32 학습은 완료됐다. dashboard v1.14가 운영 중이며 v1.15와 20문장 비교·최신 계획은 PR #28로 `master` 병합 완료·운영 미배포 상태다. 원본 프로젝트 폴더의 `master`에서 후속 작업을 진행한다. 검사기 한계·모델 오류·미측정 품질을 분리한 뒤 컨텍스트, 지시문, 큰 동일 계열 기본 모델, 데이터/입력 원인을 비교한다. 구현 상세나 실행 명령을 이 문서에 중복 정의하지 않는다.
