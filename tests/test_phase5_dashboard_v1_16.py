# test_phase5_dashboard_v1_16.py - v1.16 사전 차단·원출력 보존·세션 identity 격리를 검증한다.
...
from scripts.training.dashboard_grounding_v3 import SCORER_VERSION
...
class DashboardV116Tests(unittest.TestCase):
    def setUp(self):
        # CPU canary: 실제 GPU lock·모델 호출은 사용하지 않는다.
        patcher = patch(
    ...
    def test_http_date_block_happens_before_runner_and_rejects_client_clock(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            try:
                with (
                    patch(
                        "scripts.training.dashboard_grounding_v3.kst_today",
    ...
    def test_candidate_config_preserves_defaults_and_old_version(self):
        ...
        self.assertEqual(config["server"]["port"], 8769)
        ...
        from scripts.training import phase5_dashboard_v1_15 as previous
