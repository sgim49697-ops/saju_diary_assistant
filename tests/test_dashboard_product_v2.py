# test_dashboard_product_v2.py - R16 제품 후보의 CPU 사실 조회·입력 선택·저장 출처·동시성을 검증한다.
...
from scripts.training import dashboard_product_session_v2 as sessions
...
class ProductSessionTests(unittest.TestCase):
    ...
    def test_fresh_process_direct_answer_does_not_import_gpu_modules(self):
        result = subprocess.run([sys.executable, "-B", "-c", "from tests.test_dashboard_product_v2 import fresh_probe; fresh_probe()"], cwd=ROOT, capture_output=True, text=True, timeout=30, check=False)
