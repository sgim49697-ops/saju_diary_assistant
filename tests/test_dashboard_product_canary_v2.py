# test_dashboard_product_canary_v2.py - 제품 CPU 증거의 전량 집계·불변 발행·실패 처리를 검증한다.
...
from scripts.evaluation import dashboard_product_canary_v2 as canary
...
class ProductCanaryTests(unittest.TestCase):
    ...
    def test_failed_missing_or_skipped_tests_are_not_success(self):
        for code, output in ((1, "Ran 56 tests in 1s\nOK\n"), (0, "OK\n"), (0, "Ran 55 tests in 1s\nOK\n"), (0, "Ran 56 tests in 1s\nOK (skipped=1)\n")):
    ...
    def test_missing_failed_or_live_browser_is_not_success(self):
        for result in (subprocess.CompletedProcess([], 1, "", ""), subprocess.CompletedProcess([], 0, json.dumps({"passed": True, "cases_passed": 31}), ""), subprocess.CompletedProcess([], 0, json.dumps({"passed": True, "cases_passed": 32, "model_generations": 1, "production_service_accessed": False}), "")):
