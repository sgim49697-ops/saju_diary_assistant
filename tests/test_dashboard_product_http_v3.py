# test_dashboard_product_http_v3.py - 제품 후보의 실제 HTTP·worker·binding 경계와 CPU 실행을 검증한다.
...
from tests.test_dashboard_product_v3 import ROOT, context_fixture, synthetic_app
...
def http_app(context, *, owner=None, runner=None):
    ...
    server = dashboard.DashboardHTTPServer(("127.0.0.1", 0), context, dashboard.V120_ASSET_ROOT, "test-csrf", chart_only_binding=owner, chart_only_runtime_requested=owner is not None, generation_runner=runner)
...
class ProductHTTPTests(unittest.TestCase):
    ...
    def test_authenticated_candidate_startup_and_separate_default_port(self):
        with synthetic_app() as (context, generate, _), patch.object(dashboard, "DashboardHTTPServer") as factory, patch.object(dashboard, "_remote_access_settings", return_value=("https://synthetic.invalid", ("synthetic", "test-only-password"))), redirect_stdout(io.StringIO()):
            context["chart_only_runtime"] = {"asset_root": dashboard.V120_ASSET_ROOT}
            factory.return_value.server_address = ("127.0.0.1", 8772)
            dashboard.serve(context, "127.0.0.1", 8772)
            ...
            self.assertEqual(dashboard._parser().parse_args(["serve"]).port, 8772)
            generate.assert_not_called()
    ...
    def test_review_ordinary_terms_and_definition_remain_general_over_http(self):
        with synthetic_app() as (context, generate, _):
            owner, _ = binding_owner()
            with http_app(context, owner=owner, runner=dashboard.execute_manual_generation) as (_, base):
                for prompt in ("일간 연락드릴게요", "우리 반 일진이 누구야?", "용신이라는 용어는 무슨 의미야? 나는 사실 잘 몰라서."):
                    with self.subTest(prompt=prompt):
                        result = post(base, prompt)
                        self.assertEqual(result["response_kind"], "model_generated")
                        messages = generate.call_args.args[-1]
                        self.assertEqual(messages[0]["content"], policy.BASE_PROMPT)
    ...
    def test_review_compound_lookup_reaches_worker_without_request_loss(self):
        with synthetic_app() as (context, generate, _):
            owner, _ = binding_owner()
            with http_app(context, owner=owner, runner=dashboard.execute_manual_generation) as (_, base):
                for prompt in ("내 일간이 뭐고 성격도 알려줘.", "내 일간이 뭐고 오늘 기분이 안 좋은데 위로도 해줘."):
                    with self.subTest(prompt=prompt):
                        result = post(base, prompt)
                        self.assertEqual(result["response_kind"], "model_generated")
                        messages = generate.call_args.args[-1]
                        self.assertIn("[현재 승인 사실]", messages[0]["content"])
                        self.assertIn("丙", messages[0]["content"])
                        self.assertEqual(messages[-1]["content"], prompt)
                        stored = dashboard.manual_session_payload(context, result["session_id"])
                        self.assertEqual(stored["messages"][-1]["content"], result["output"])
                        self.assertEqual(stored["messages"][-1]["response_kind"], "model_generated")
    ...
    def test_real_cli_worker_direct_answer_with_no_model_artifacts(self):
        with tempfile.TemporaryDirectory(prefix="saju-product-cli-") as directory:
            ...
            command = [sys.executable, "-B", "-m", "scripts.training.phase5_dashboard_v1_20", "--artifact-root", str(root), "--run-root", str(run), "generate", "--execute", "--enable-chart-only-runtime-binding"]
