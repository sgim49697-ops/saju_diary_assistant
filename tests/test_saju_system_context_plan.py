...
class SajuSystemContextPlanTests(unittest.TestCase):
    ...
    def test_system_stages_remain_inside_the_existing_four_phases(self) -> None:
        ...
        self.assertIn("S4는 v1.1에서 `executed`·공개 build `verified`", self.plan)
        self.assertIn("기존 1오류·191미실행을 성공이나 품질 실패 점수로 바꾸지 않는다", self.plan)
        self.assertIn("S5는 CPU 분석 `executed`·공개 build `verified`", self.plan)
        self.assertIn("S6은 `not_executed`", self.plan)
        self.assertIn("build-27a91a8e21a8", self.plan)
