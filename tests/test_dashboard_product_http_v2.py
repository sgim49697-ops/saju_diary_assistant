# test_dashboard_product_http_v2.py - 제품 후보의 실제 HTTP·worker·binding 경계와 CPU 실행을 검증한다.
...
from tests.test_dashboard_product_v2 import ROOT, context_fixture, synthetic_app
...
def http_app(context, *, owner=None, runner=None):
    ...
    server = dashboard.DashboardHTTPServer(("127.0.0.1", 0), context, dashboard.V119_ASSET_ROOT, "test-csrf", chart_only_binding=owner, chart_only_runtime_requested=owner is not None, generation_runner=runner)
...
class ProductHTTPTests(unittest.TestCase):
    ...
    def test_authenticated_candidate_startup_and_separate_default_port(self):
        with synthetic_app() as (context, generate, _), patch.object(dashboard, "DashboardHTTPServer") as factory, patch.object(dashboard, "_remote_access_settings", return_value=("https://synthetic.invalid", ("synthetic", "test-only-password"))), redirect_stdout(io.StringIO()):
            context["chart_only_runtime"] = {"asset_root": dashboard.V119_ASSET_ROOT}
            factory.return_value.server_address = ("127.0.0.1", 8771)
            dashboard.serve(context, "127.0.0.1", 8771)
            ...
            self.assertEqual(dashboard._parser().parse_args(["serve"]).port, 8771)
            generate.assert_not_called()
    ...
    def test_real_cli_worker_direct_answer_with_no_model_artifacts(self):
        with tempfile.TemporaryDirectory(prefix="saju-product-cli-") as directory:
            ...
            command = [sys.executable, "-B", "-m", "scripts.training.phase5_dashboard_v1_19", "--artifact-root", str(root), "--run-root", str(run), "generate", "--execute", "--enable-chart-only-runtime-binding"]
